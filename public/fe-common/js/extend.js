define(function(require, exports, modules) {
    require("ui-blocklist");
    require("ui-pagination");
    require("ui-selectgroup");
    require("ui-logicblock");
    require('ui-notice');
    require('ui-htree');
});
...
define("ui-pagination",function(require, exports, module){
    ...
    $.aeWidget('ae.aePagination',{
         ...
         * @property {boolean} options.isShowTotalPage          - 是否显示总页数，默认为false
         * @property {boolean} options.isShowCurrPage           - 是否显示当前页，默认为false
         * @property {boolean} options.isShowTotalRecords       - 是否显示总记录数，默认为false
         * @property {boolean} options.isMinPagination          - 是否显示为mini翻页模式，默认为false
         * @property {function} options.onPageChange            - 翻页事件回调函数，默认为null
         */
        options:{
            initType: "html",
            totalRecords: 0,
        ...
        _create: function(){
            ...
            if(options.initType == 'html'){
                $ele.addClass("wrapper clearfix");
                self._buildOptions(options, $ele);
            }
        },
        _buildOptions:function(ops,el){
            ops.aeType = el.attr("aeType") || "aePagination";
            ...
            ops.isShowTotalPage = el.attr("isShowTotalPage") || ops.isShowTotalPage;
            ops.isShowCurrPage = el.attr("isShowCurrPage") || ops.isShowCurrPage;
            ops.isShowTotalRecords = el.attr("isShowTotalRecords") || ops.isShowTotalRecords;
            ops.isGoPage = el.attr("isGoPage") || ops.isGoPage;
            ops.onPageChange = el.attr("onPageChange") || ops.onPageChange;
            ops.align = el.attr("align") || ops.align;
        },
        /**
         * 根据选项重新加载组件
        ...
        _generPageHtml : function(ops){
            ...
            ops.prv = (ops.currentPage <= 2) ? 1 : (ops.currentPage-1);
            ops.next = (ops.currentPage >= ops.totalPage-1) ? ops.totalPage : (ops.currentPage + 1);
            ops.hasPrv = (ops.currentPage > 1);
            ops.hasNext = (ops.currentPage < ops.totalPage);

            var str_first='',str_prv='',str_next='',str_last='';
            if(ops.isShowFirstLastBtn){
                if(ops.hasPrv){
                    str_first = '<li><a '+this._getHandlerStr(1)+' title="' + i18n.firstPageTipText + '">\<\<</a></li>';
                }else{
                    str_first = '<li class="disabled"><a href="javascript:void(0)" title="' + i18n.firstPageTipText + '">\<\<</a></li>';
                }
                if(ops.hasNext){
                    str_last = '<li><a '+this._getHandlerStr(ops.totalPage)+' title="' + i18n.lastPageTipText + '">\>\></a></li>';
                }else{
                    str_last = '<li class="disabled" title="' + i18n.lastPageTipText + '"><a href="javascript:void(0)">\>\></a></li>';
                }
            }
            if(ops.isShowPreNextBtn){
                if(ops.hasPrv){
                    str_prv = '<li><a '+this._getHandlerStr(ops.prv)+' title="' + i18n.prePageTipText + '">\<</a></li>';
                }else{
                    str_prv = '<li class="disabled" title="' + i18n.prePageTipText + '"><a href="javascript:void(0)">\<</a></li>';
                }
                if(ops.hasNext){
                    str_next = '<li><a '+this._getHandlerStr(ops.next)+' title="' + i18n.nextPageTipText + '">\></a></li>';
                }else{
                    str_next = '<li class="disabled"><a href="javascript:void(0)" title="' + i18n.nextPageTipText + '">\></a></li>';
                }
            }
            var str = '';
            var dot = '<li class="dot"><a href="javascript:void(0)">...</a></li>';

            //分页处理
            //如果为MinPagination
            if(ops.isMinPagination){
                str='<li class="active"><a href="javascript:void(0)">'+ops.currentPage+'</a></li>';
            }else{
                if(ops.totalPage <= 8){
                    for(i=1;i<=ops.totalPage;i++){
                        if(ops.currentPage == i){
                            str += '<li class="active"><a href="javascript:void(0)">'+i+'</a></li>';
                        }else{
                            str += '<li><a '+this._getHandlerStr(i)+'>'+i+'</a></li>';
                        }
                    }
                }else{
                    if(ops.currentPage <= 5){
                        for(i=1;i<=6;i++){
                            if(ops.currentPage == i){
                                str += '<li class="active"><a href="javascript:void(0)">'+i+'</a></li>';
                            }else{
                        ...
                        str += dot;
                        str += '<li><a '+this._getHandlerStr(ops.totalPage)+'>'+ops.totalPage+'</a></li>';
                    }else{
                        str += '<li><a '+this._getHandlerStr(1)+'>1</a></li>';
                        str += dot;

                        var begin = ops.currentPage - 2;
                        var end = ops.currentPage + 2;
                        if(end > ops.totalPage){
                            end = ops.totalPage;
                            begin = end - 4;
                            if(ops.currentPage - begin < 2){
                                begin = begin-1;
                            }
                        }else if(end + 1 == ops.totalPage){
                            end = ops.totalPage;
                        }
                        for(i=begin;i<=end;i++){
                            if(ops.currentPage == i){
            ...
            var pagerHtml = '<ul class="pagination m-n pull-right">';
            pagerHtml += str_first + str_prv + str + str_next + str_last;
            pagerHtml += '</ul>';

            var html = "", flag, id, changeName;
            if(ops.isShowPageSizeCombo){
                flag = true;
                id = this.element.attr("id")+"_PageSize";
                changeName = id.replace(/-/g, "_");
                if(typeof ops.onComboChange == "function"){
                    html = '<div class="pull-left m-r-sm"><span onValueChange="'+changeName+'" aeType="aeCombo" id="'+id+'" aeInit="true"width="100"></span></div>';
                }else{
                    html = '<div class="pull-left m-r-sm"><span aeType="aeCombo" id="'+id+'" aeInit="true"width="100"></span></div>';
                }
            }
            var total_info='';
            ...
            if(flag){
                ...
                for(key in data){
                    if(data[key].value == ops.pageSize){
                        comboFlag = true;
                        break;
                    }
                }
                if(!comboFlag){
                    data.unshift({"text":(ops.pageSize+" Items"),"value": ops.pageSize});
                }

                if(typeof ops.onComboChange == "function"){
                    window[changeName] = function(){
                        var pageSize = $("#"+id).aeCombo("getValue");
                        self.options.pageSize = pageSize;
                        self.options.currentPage = "1";
                        ops.onComboChange.call(self, self.options.currentPage, pageSize);
                        self.setTotalRecords(self.options.totalRecords);
                    };
                }

                $("#"+id).aeCombo("reload",data);
                $("#"+id).aeCombo("setDefault",ops.pageSize);
            }

            this.element.find("li:not(.disabled,.active,.dot)").on("click",function(){
                self.selectPage($(this).children().attr("data-value"));
                self.options.onPageChange.call(self, $(this).children().attr("data-value"), ops.pageSize);
            });
        },
        ...
        /**
         * 设置总记录数
         * @name ae.aePagination#setTotalRecords
         * @function
    ...
    $.aeWidget('ae.aeBlocklist', {
        ...
        _loadColumnHead:function(){
            var options = this.options,
                $el,
                $cntr;
            if(options.columnHeadId){
                $el=$("#"+options.columnHeadId);
                if($el.length>0){
                    $cntr=this.element.find("[data-role='column-head-container']");
         ...
         * @private
         */
        _createExceptionBrand:function(){
            var $ele = this.element,
                $emptyHtml;
            if($ele.find("[data-role='exceptionBrand']").length===0){
                $emptyHtml = $('<div data-role="exceptionBrand" style="display: none;width:100%" class="text-center padder-v-lg m-t-xl m-b-lg bg-white text-muted"> <i class="md-warning icon-xl"></i><p class="text-md">Data error.</p></div>');
                $emptyHtml.appendTo($ele);
            }
        },
        _showExceptionBrand:function(){
            if(this.options.showException){
                this.element.find("[data-role='exceptionBrand']").fadeIn();
            }
        },
        _hideExceptionBrand:function(){
            this.element.find("[data-role='exceptionBrand']").stop().hide();
        },
        _reload:function(data){
            ...
            this._hideExceptionBrand();
            this._hideEmptyBrand();
            this._rebuildSkeleton();
            this._loadColumnHead();
            if(!dataArray){
                this._createEmptyBrand();
        ...
        _rebuildSkeleton:function(){
            var $ele=this.element;
            if(this.options.columnHeadId){
                if($ele.find(".block-list-tb").length===0){
                    this.element.empty().append($("<div class='block-list-tb'><div data-role='column-head-container' class='list-head'></div><div data-role='body-container' class='list-body'></div></div>"));
                }
            }else{
                if($ele.find(".block-list").length===0){
                    this.element.empty().append($("<div class='block-list'><div data-role='column-head-container'></div><div data-role='body-container'></div></div>"));
                }
            }
        },
        /**
         * 重新加载数据
         * @name ae.aeBlocklist.reload
         * @function
        ...
        _showPagination:function(){
            ...
            var $page = $("#"+this.options.pagingConfig.id);
            if($page.length>0){
                $page.show();
            }
        },
        ...
        _setRowSelectedStyle:function(chk,isChecked){
            ...
            if(isChecked){
                $(chk.parentNode.parentNode).addClass(opts.selectedRowStyle);
            }else{
                $(chk.parentNode.parentNode).removeClass(opts.selectedRowStyle);
            }
        },
        _addEvents:function(){
            var opts=this.options,
            ...
            if(opts.selectedRowStyle){
                this.element.off("click").on("click","[data-role='_checkbox']",function(event){
                    self._setRowSelectedStyle(this,this.checked);
                });
            }
            if(opts.traceRowStyle){
                this.element.find(".list-item").on("mouseover",function(event){
                    var $this=$(this),
                        $chk=$this.find("[data-role='_checkbox']");
                    if(opts.selectedRowStyle && $chk.length>0 && $chk[0].checked){
                        $this.removeClass(opts.selectedRowStyle);
                    }
                    $this.addClass(opts.traceRowStyle);
                }).on("mouseout",function(event){
                    var $this=$(this),
                        $chk=$this.find("[data-role='_checkbox']");
                    $this.removeClass(opts.traceRowStyle);
                    if(opts.selectedRowStyle && $chk.length>0 && $chk[0].checked){
                        $this.addClass(opts.selectedRowStyle);
                    }
                });
            }
        },
        /**
         * 清空列表内容
         * @name ae.aeBlocklist.clear
         * @function
         */
        clear:function(){
            this.element.find("[data-role='body-container']").empty();
            this._showEmptyBrand();
        },
        /**
         * 选择��?有项目，此方法只在多选模式是有效
        ...
        selectAll:function(){
            ...
            if(this.options.mutiSelect){
                $chks=this.element.find("[data-role='_checkbox']");
                count=$chks.length;
                for (i = 0; i < count; i++) {
                    if($chks[i].checked===false){
                        $chks[i].checked=true;
                        this._setRowSelectedStyle($chks[i],true);
                    }
                }
            }
        },
        ...
        getSelected:function(){
            var $els,
                ...
                count;
            if(this.options.mutiSelect){
                $els=this.element.find("[data-role='_checkbox']");
                for (i = 0; i < $els.length; i++) {
                    if($els[i].checked){
                        $item=$($els[i].parentNode.parentNode);
                        data.push($item.data());
                    }
                }
            }else{
         ...
         */
        hasPaging: function () {
            var pagingConfig = this.options.pagingConfig;
        ...
        addBlock: function (blockData, tmpl, pos) {
            ...
            if ($.isArray(blockData)) {
                arrayData = blockData;
            } else if ($.isObject(blockData)) {
                arrayData.push(blockData);
            }
            if (this.hasPaging() === false) {
                this._hideExceptionBrand();
                this._hideEmptyBrand();
                if (tmpl) {
                    template = Handlebars.compile("<div class='list-item'>" + tmpl + "</div>");
                }
                resultHtmlObj = this._generatorDOMs(arrayData, template);
                count=resultHtmlObj.length;
                if (pos && pos === "top") {
                    for (i=0;i<count;i++) {
                        resultHtmlObj[i].prependTo($ele);
                    }
                } else {
                    for (i=0;i<count;i++) {
                        resultHtmlObj[i].appendTo($ele);
                    }
                }
                $.aries.common.globalInit(this.element.attr("id"));
                return (resultHtmlObj.length === 1 ? resultHtmlObj[0] : resultHtmlObj);
            }
            return null;
        },
        /**
        ...
        _buildOptions: function (options, element) {
            var tmplStr;
            options.aeType = element.attr("aeType") || "aeBlocklist";
            options.col = options.col || element.attr("col");
            options.selectedRowStyle =  options.selectedRowStyle || element.attr("selectedRowStyle");
            options.showEmpty = options.showEmpty || element.attr("showEmpty");
            options.showEmpty = ((options.showEmpty==="false")?false:(!!options.showEmpty));
            options.showException = options.showException || element.attr("showException");
            options.showException = ((options.showException==="false")?false:(!!options.showException));
            options.emptyTipMsg = element.attr("emptyTipMsg") ? element.attr("emptyTipMsg") : "data";
            options.mutiSelect = options.mutiSelect || element.attr("mutiSelect");
            options.mutiSelect = ((options.mutiSelect==="false")?false:(!!options.mutiSelect));
            if(options.mutiSelect){
                tmplStr="<div class='list-item' style='display: none;position:relative;padding-left:22px'><div style='position:absolute;margin-left:5px;left:0;top:50%;margin-top:-12px'><input type='checkbox' data-role='_checkbox' style='cursor:pointer'/></div>"+options.tpl+"</div>";
            }else{
                tmplStr="<div class='list-item' style='display: none'>"+options.tpl+"</div>";
            }
            options.template = Handlebars.compile(tmplStr);
            options.columnHeadId=options.columnHeadId || element.attr("columnHeadId") || "";
        },

        _generator : function(data){
            var options = this.options;
         ...
         * 根据数据生成对应的html结构
         * @private
         * @param  {Array} srcArr 数据
         */
        _generatorHtml: function (srcArr) {
            var i,
                count,
                $ele = this.element.find("[data-role='body-container']"),
                options=this.options,
                resultHtmlObj = this._generatorDOMs(srcArr);
            for (i=0,count=resultHtmlObj.length;i<count;i++) {
                resultHtmlObj[i].appendTo($ele).fadeIn();
            }
        },
        /**
         * 创建空数据视��?
        ...
        _createEmptyBrand:function(){
            var $ele = this.element,
                options = this.options,
                $emptyHtml;
            if($ele.find("[data-role='emptyBrand']").length===0){
                $emptyHtml = $('<div style="display: none;width:100%" data-role="emptyBrand" class="text-center padder-v-lg m-t-xl m-b-lg bg-white text-muted"> <i class="md-error icon-xl"></i><p class="text-md">No ' + options.emptyTipMsg + '.</p></div>');
                $emptyHtml.appendTo($ele);
            }
        },
        _showEmptyBrand:function(){
            if(this.options.showEmpty){
                this.element.find("[data-role='emptyBrand']").fadeIn();
            }
        },
        _hideEmptyBrand:function(){
        ...
        _generatorDOMs: function (data, tpl) {
            ...
            if (col == "1") {
                for (i=0;i<count;i++) {
                    itemStr = template(data[i]);
                    $item = $(itemStr);
                    $item.data(data[i]);
                    doms.push($item);
                }
            } else {
                rowStr = "<div class='list-row'></div>";
                $row = $(rowStr);
                for (i=0;i<count;i++) {
                    itemStr = template(data[i]);
                    $item = $(itemStr);
...
define('ui-selectgroup',function (require, exports, moudles) {
    $.aeWidget('ae.aeSelectgroup', {
        options:{
            initType: "html",
            /**
        ...
        _create : function(){
            ...
            if(options.initType == 'html'){
                options.mode = $ele.attr("mode") || options.mode;
                options.aeType = $ele.attr("aeType") || "aeSelectgroup";
            }
        },
        _init : function(){
            var self = this, options = self.options, el = self.element;
            var arrayHtml = [],itemHtml="";
            if(options.mode === "complex"){
                el.addClass("btn-group select-group select-group-complex");
            }else{
                el.addClass("btn-group select-group select-group-simple");
            }

            var labelArr = el.find(">button");
            labelArr.each(function(index, item){
                $(item).addClass("btn btn-default");
                if($(item).find("input").next().length === 0){
                    var html = $(item).html();
            ...
            var type = self.type();
            (function(){
                el.on("click", "button.btn",function(e){
                    var checked = $(this).find("input").is(":checked");
                    if(type === "radio"){
                        if(!checked){
                            $(this).siblings(".btn-primary")
                            .removeClass("btn-primary").addClass("btn-default")
                            .find("i.fa-check-circle-o").remove();
                            $(this).addClass("btn-primary").removeClass("btn-default");
                            if(options.mode === "complex"){
                                $(this).append("<i class=\"fa fa-check-circle-o\"></i>");
                            }
                            $(this).find("input").attr("checked",true);
                        }
                    }else{
                        if(checked){
                            $(this).removeClass("btn-primary").addClass("btn-default").find("i.fa-check-circle-o").remove();
        ...
        getSelectValue : function(){
            ...
            if(this.type() === "radio"){
                ...
                var checkedArr = [];
                el.find("input[type='checkbox']").each(function(index, item){
        ...
        getSelectText : function(){
            var self = this, options = self.options, el = self.element;
            if(this.type() === "radio"){
                return el.find("input[type='radio']:checked").next().text();
            }else{
                var checkedArr = [];
                el.find("input[type='checkbox']").each(function(index, item){
                    if($(item).prop("checked")){
                        checkedArr.push($(item).next().text());
                    }
                });
                return checkedArr;
            }
            ...
            if(!isNaN(obj)){
                if( type=== "radio"){
                    el.find(".btn-primary")
                        .removeClass("btn-primary").addClass("btn-default")
                        .find("i.fa-check-circle-o").remove();
                    selectObj = el.children().eq(parseInt(obj));
                    selectObj.addClass("btn-primary").removeClass("btn-default");
                    if(options.mode === "complex"){
                        selectObj.append("<i class=\"fa fa-check-circle-o\"></i>");
                    }
                    el.find("input[type='radio']").eq(parseInt(obj)).attr("checked", true);
                }else if(type === "checkbox"){
                    selectObj = el.find(".btn").eq(parseInt(obj));
                    if(selectObj.hasClass("btn-primary")) return false;
                    selectObj.addClass("btn-primary").removeClass("btn-default");
                    if(options.mode === "complex"){
                        selectObj.append("<i class=\"fa fa-check-circle-o\"></i>");
                    }
                    selectObj.find("input").prop("checked","checked");
                }
            }else if(Object.prototype.toString.call(obj) == "[object Array]"){
                if(type === "radio") return false;
                for (var i = 0; i < obj.length; i++) {
                    selectObj = el.find(".btn").eq(parseInt(obj[i]));
                    if(selectObj.hasClass("btn-primary")) continue;
                    selectObj.addClass("btn-primary").removeClass("btn-default");
                    if(options.mode === "complex"){
                        selectObj.append("<i class=\"fa fa-check-circle-o\"></i>");
                    }
                    el.find("input").eq(parseInt(obj[i])).prop("checked","checked");
                }
            }
        },
        /**
        ...
        setSelectByValue : function(obj){
            ...
            if(!isNaN(obj)){
                if(type === "radio"){
                    el.find(".btn-primary").removeClass("btn-primary").addClass("btn-default").find("i.fa-check-circle-o").remove();
                    selectObj = el.find("input[value='" + obj + "']:radio")
                        .attr("checked", true)
                        .parent().addClass("btn-primary").removeClass("btn-default");
                    if(options.mode === "complex"){
                        selectObj.parent().append("<i class=\"fa fa-check-circle-o\"></i>");
                    }
                }else if(type === "checkbox"){
                    selectObj = el.find("input[value='" + obj + "']");
                    if(selectObj.hasClass("btn-primary")) return false;
                    selectObj.prop("checked","checked")
                    .parent().addClass("btn-primary").removeClass("btn-default");
                    if(options.mode === "complex"){
                        selectObj.parent().append("<i class=\"fa fa-check-circle-o\"></i>");
                    }
                ...
                if(type === "radio") return false;
                for (var i = 0; i < obj.length; i++) {
                    selectObj = el.find("input[value='" + obj[i] + "']");
                    if(selectObj.hasClass("btn-primary")) return false;
                    selectObj.prop("checked","checked")
                    .parent().addClass("btn-primary").removeClass("btn-default");
                    if(options.mode === "complex"){
        ...
        empty : function(){
            var self = this, options = self.options, el = self.element;
            el.find(".btn-primary").removeClass("btn-primary").addClass("btn-default");
            if(this.type() === "radio"){
                return el.find("input[type='radio']:checked").removeAttr("checked");
            }else{
                el.find("input[type='checkbox']").each(function(index, item){
                    if($(this).prop("checked")){
                        $(this).removeAttr("checked");
                        $(this).parent().find("i.fa-check-circle-o").remove();
                    }
                });
            }
            return false;
...
/**
 * 表达式可视化编辑组件模块
 * @module ui-logicblock
 * @requires ui-draggable
 * @requires ui-droppable
 * @requires ui-flip
 * @requires ui-calendar
 * @requires ui-tree
 * @requires ui-combo
 */
define('ui-logicblock', function(require, exports, moudles) {
    var tmplGroup,
    ...
    $.ae.lang.aeLogicBlock={
        ...
        dropTip: "drop here add to the group",
        notFound:"No matching items."
    };
    tmplGroup = "<div class='condition-group' id='{{id}}' data-role='member-group'><div class='condition-group-title'><span class='ion-drag pos-abt' data-role='drag-handle' style='cursor:move;top:10px;left:10px;opacity:0.5;filter:\"alpha(opacity=0.5)\"'></span><span class='ion-close pos-abt' data-role='group-del' style='cursor:pointer;bottom:10px;left:12px;opacity:0.5;filter:\"alpha(opacity=0.5)\"' data-group-id='{{id}}'></span><a class='inline-block text-white v-middle font-bold' data-role='oper' data-oper='and' data-group-id='{{id}}'>AND</a></div><div class='condition-group-content' data-role='group-cntr' data-dock='true' data-group-id='{{id}}'><button class='btn btn-default btn-block' data-role='member-add' style='opacity:0.5;filter:\"alpha(opacity=0.5)\"' data-group-id='{{id}}'>" + $.ae.lang.aeLogicBlock.addNewCondition + "</button><div class='b-gray text-center text-muted b-r b-2x b-dashed' style='height:32px;display:none' data-role='member-drop-tip' data-group-id='{{id}}'><span class='ion-arrow-shrink text-md'></span>" + $.ae.lang.aeLogicBlock.dropTip + "</div></div></div>";

    function getSearchbarString(){
        var i18n=$.ae.lang.aeLogicBlock,
            s='<div class="node-selection-search"><i class="close_circle pull-right none" data-role="search-clear" style="cusor:pointer"></i><i class="search"></i><input type="text" class="node-search-input" data-role="search-input"/></div><ul class="none" data-role="result" style="padding:50px 10px 0 10px;width:100%"><div data-role="not-found" style="width:100%" class="none text-center padder-v-lg m-t-xl m-b-lg bg-white text-muted"><i class="md-info-outline text-muted" style="width:70px;height:70px"></i><p class="text-md text-muted">'+i18n.notFound+'</p></div></ul>';
        return s;
    }
    function getResultItemString(nodeId,nodeInfo) {
        var s="<li data-node-id='"+nodeInfo.id+"'><i class='icon-page2' style='font-size:24px;color:rgb(204,204,204)'></i><div style='display:inline-block;padding:5px'>";
        s+="<span title='"+nodeInfo.name+"'>"+nodeInfo.name+"</span>";
        s+="</div></li>";
        return s;
    }
    ...
    function defaultTemplateParser(attr,val,tmplStr) {
        var s;
        s=tmplStr.replace(/\$ATTR_CODE\$/g,attr);
    ...
    function getOperator(operators,operatorName) {
        ...
        for(i=operators.length-1;i>=0;i--){
            if(operators[i].name===operatorName){
                return operators[i];
            }
        }
        return null;
    ...
    function Expression(exprId, rootGroupId,defaultOperatorText,defaultOperatorCode) {
        ...
        this.root = new MemberGroup({
            id: rootGroupId || false,
            expr: this,
            operator:defaultOperatorCode,
            operatorName:defaultOperatorText
        });
        this.groups = {};
        this.groups[this.root.id] = this.root;
    }
    Expression.prototype = {
        ...
        getState: function() {
            var obj = {
                ...
                groups = this.groups;
            for(ky in groups) {
                obj.groups.push(groups[ky].getState());
            }
            return obj;
        },
        _clear: function() {
            ...
            this.id = "";
            this.root = null;
            for(ky in this.groups) {
                this.groups[ky].gc();
                delete this.groups[ky];
            }
        },
        /**
         * 设置状�??
        ...
        setState: function(state,mapVariables) {
            var i,
                ...
                ky,
                g;
            if(typeof(state) === "object" && $.isArray(state.groups) && state.groups.length > 0) {
                this._clear();
                this.id = state.id;
                ...
                for(ky in this.groups) {
                    g = this.groups[ky];
                    g.setState(groupStates[map[g.id]],mapVariables);
                }
                return true;
            }
            return false;
     ...
     * @constructor
     */
    function Member(id, variableName, variable,variableId, operator,operatorName, value,valueText, valueType, controlType, template,displayTemplate, parentGroup) {
        this.id = id;
    ...
    function MemberGroup(options) {
        ...
        this.operatorName = options.operatorName || "AND";
        this.members = options.members || [];
        this.id = options.id || getId("g");
    ...
    MemberGroup.prototype = {
        getState: function() {
            var state = {
                    ...
                    operator: this.operator,
                    operatorName:this.operatorName,
                    parentGroupId: (this.parentGroup === null) ? "" : this.parentGroup.id
                },
                ms = this.members,
                i,
                count = ms.length;
            for(i = 0; i < count; i++) {
                if(ms[i] instanceof MemberGroup) {
                    state.members.push({
                        id: ms[i].id,
                        isGroup: "1"
                    });
                } else {
                    state.members.push(ms[i].getState());
                }
            }
            return state;
        },
        setState: function(state,mapVariables) {
            ...
            this.operatorName = state.operatorName;
            this.id = state.id;
            this.parentGroup = this.expr.getGroup(state.parentGroupId);
            ms = state.members;
            this.members = [];
            count = ms.length;
            for(i = 0; i < count; i++) {
                if(ms[i].isGroup === "1") {
                    group = this.expr.getGroup(ms[i].id);
                    this.members.push(group);
                } else {
                    member = new Member(ms[i].id,
                        ms[i].variableName,
                        ms[i].variable,
                        ms[i].variableId,
                        ms[i].operator,
                        ...
                        ms[i].value,
                        ms[i].valueText,
                        parseInt(ms[i].valueType,10),
                        parseInt(ms[i].controlType,10),
                        ms[i].template,
                        ms[i].displayTemplate,
                        this);
        ...
        toExpression: function(assembler,useLogicOperatorText) {
            ...
            for(i = 0; i < count; i++) {
                if(ms[i] instanceof MemberGroup) {
                    s += ms[i].toExpression(assembler,useLogicOperatorText);
                } else {
                    if(typeof(assembler) === "function") {
                        userData=mapVariables[ms[i].variableId].userData;
                        result = assembler({
                            "variableCode":ms[i].variable,
                            "variableName":ms[i].variableName,
                            "operatorCode":ms[i].operator,
                            "operatorName":ms[i].operatorName,
                            "value":ms[i].value,
                            "valueText":ms[i].valueText,
                            "valueType":ms[i].valueType,
                            "template":ms[i].template,
                            "displayTemplate":ms[i].displayTemplate,
                            "templateParser":opts.templateParser,
                            "userData":userData
                        });
                        if(typeof(result) === "string") {
                            s += result;
                        }else if(result===false){
                            tmplStr=ms[i].template;
                            if(typeof(tmplStr)==="string" && tmplStr!==""){
                                exprStr=tmplParser(ms[i].variable ,ms[i].value,tmplStr);
                            }else{
                                exprStr=ms[i].variable+ms[i].operator+ms[i].value;
                            }
                            s += " " + exprStr + " ";
                        }
                    } else {
                ...
                if(i !== (count - 1)) {
                    s += " " + ((useLogicOperatorText)?this.operatorName:this.operator) + " ";
                }
            }
         ...
         * @returns {number} 如果存在，返回此成员对应的序号，否则返回-1
         */
        hasMember: function(member) {
            var ms = this.members,
                ...
                count,
                id;
            if((member instanceof Member) || (member instanceof MemberGroup)) {
                id = member.id;
                for(i = 0, count = ms.length; i < count; i++) {
                    if(ms[i].id === id) {
                        return i;
                    }
                }
         ...
         * @returns {Member|MemberGroup} 返回存在的Member实例或MemberGroup实例，如果没找到成员返回null
         */
        ...
        addMember: function(member) {
            var idx,
                isMemberGroup = (member instanceof MemberGroup);
            if((member instanceof Member) || isMemberGroup) {
                idx = this.hasMember(member);
                if(idx === -1) {
                    if(member.parentGroup instanceof MemberGroup) {
                        member.parentGroup.removeMember(member);
                    }
        ...
        gc: function() {
            ...
            if(this.parentGroup !== null) {
                this.parentGroup.removeMember(this);
                this.parentGroup = null;
            }
            this.expr = null;
        }
    ...
    var ExpressionHelper = {
        ...
        addGroup: function(groupId, parentGroupId) {
            ...
            group = new MemberGroup({
                id: groupId,
                expr: expr,
                operatorName:defaultLogicOperator.text,
                operator:defaultLogicOperator.code
            });
            parentGroup = expr.getGroup(parentGroupId);
            expr.addGroup(group, parentGroup);
            return group;
        },
        /**
         * 根据组Id获取表达式对��?
    ...
    function getCondition(memberId, valueType) {
        var $el = $("#" + memberId),
            groupId = $el.attr("data-group-id"),
            $variable,
            variableInfo,
            mapVars,
            opts = getOptions(groupId),
            $operator,
            operatorInfo,
            controlType,
            $enum,
        ...
        $operator=$el.find("[data-role='member-operator']");
        controlType=parseInt($operator.aeCombo("getUserdata",$operator.aeCombo("getIndex")).controlType,10);

        if(controlType == CTRL_ENUM) {
            $enum=$el.find("[data-role='member-value-enum']");
            v = $enum.aeCombo("getValue");
            vt=$enum.aeCombo("getDisplayText");
        } else if(controlType == CTRL_TEXT) {
            v = $el.find("[data-role='member-value']").val();
            vt=v;
        } else if(controlType == CTRL_NUM) {
            v = $el.find("[data-role='member-value-number']").aeFlip("getValue");
            vt=v;
        } else if(controlType == CTRL_DATE) {
            if(valueType === 3) {
                v = $el.find("[data-role='member-value-date']").aeCalendar("getValue");
                vt=v;
            }else if(valueType === 4) {
        ...
        obj = {
            "valueType": valueType,
            "operator": $operator.aeCombo("getValue"),
            "operatorName": $operator.aeCombo("getDisplayText"),
            "controlType": controlType,
            "value": v || "?",
            "valueText":vt || "?"
        };
        if(opts.hasCatalog) {
            $variable = $el.find("[data-role='member-tree-button']");
            obj.variable = $variable.attr("data-code");
            mapVars=getMapVars(groupId);
            variableInfo=mapVars[$variable.attr("data-variable-id")];
            obj.variableName = variableInfo.name;
            obj.variableId = variableInfo.id;
            operatorInfo=getOperator(variableInfo.operators,obj.operatorName);
            obj.template=operatorInfo.template || "";
            obj.displayTemplate=operatorInfo.displayTemplate || "";
        } else {
            $variable = $el.find("[data-role='member-variable']");
            obj.variable = $variable.aeCombo("getValue");
            idx = $variable.aeCombo("getIndex");
            variableInfo=$variable.aeCombo("getUserdata",idx);
            obj.variableName = $variable.aeCombo("getDisplayText");
            obj.variableId = variableInfo.id;
            operatorInfo=getOperator(variableInfo.operators,obj.operatorName);
            obj.template=operatorInfo.template || "";
            obj.displayTemplate=operatorInfo.displayTemplate || "";
    ...
    function createMemberGroupView(groupId) {
        var s;
        groupId = groupId || getId("gid");
        s = tmplGroup.replace(/\{\{id\}\}/g, groupId);
        return $(s);
    }

    function createMemberView(groupId, memberId) {
        var s;
        memberId = memberId || getId("mid");
        s = tmplMember.replace(/\{\{id\}\}/g, memberId);
        s = s.replace(/\{\{group_id\}\}/g, groupId);
        return $(s);
    }

    function addOperatorItems($select, data, selectedValue) {
        var i,
            count,
            selected;
        count = data.length;
        for(i = 0; i < count; i++) {
            selected = (data[i].name === selectedValue) ? "selected" : "";
            $select.append($("<option value='" + data[i].code + "'" + selected + " data-control-type='"+data[i].controlType+"' data-default-value='"+data[i].defaultValue+"'>" + data[i].name + "</option>"));
        }
    }

    function addItems($select, data, selectedValue) {
        var i,
            count,
            selected;
        count = data.length;
        for(i = 0; i < count; i++) {
            selected = (data[i].name === selectedValue) ? "selected" : "";
            $select.append($("<option value='" + data[i].code + "' " + selected + ">" + data[i].name + "</option>"));
        }
    }

    function createVariableTreeButtonView(groupId, memberId, data) {
        var firstVariableId=data[0].variableId;
        var $el = $("<button class='btn btn-default m-b-xs' data-role='member-tree-button' data-variable-id='"+firstVariableId+"' data-group-id='" + groupId + "' data-member-id='" + memberId + "'></button>");
        return $el;
    }

    function createVariableTreeDialogView(exprId, data) {
        var id = "logicblock-" + exprId,
            $el=$("#"+id),
            s=getSearchbarString();

        if($el.length===0){
            $el = $("<div aeType='aeDialog' id='" + id + "' data-role='member-tree-dialog' style='display:none'>"+s+"<div style='padding-top:40px;height:100%' data-role='var-tree'><div style='overflow:auto;height:100%'><div aeType='aeTree' showCheckbox='false' showLine='true' idField='id' pIdField='pid' labelField='name' valueField='id' data-role='member-tree' onSelect='hdVariablesTreeSelect'></div></div></div></div>");
            $(document.body).append($el);
        }
        return $el;
    }

    function addVariableItems($select, data, selectedValue) {
        var i,
            count,
            selected,
            $el,
        ...
        for(i = 0; i < count; i++) {
            if(!data[i].pid) {
                $el = $("<optgroup label='" + data[i].name + "'></optgroup>");
                optgroups[data[i].id] = $el;
                groupsCount++;
            }
        }
        noLeaf = (groupsCount === count);
        for(i = 0; i < count; i++) {
            selected = (data[i].name === selectedValue) ? "selected" : "";
            if(noLeaf) {
                $el = $("<option value='" + data[i].code + "' data-idx='" + i + "' " + selected + " data-value-type='" + (data[i].valueType || 1) + "'>" + data[i].name + "</option>");
                $select.append($el);
            } else {
                if(data[i].pid) {
                    $el = $("<option value='" + data[i].code + "' data-idx='" + i + "' " + selected + " data-value-type='" + (data[i].valueType || 1) + "'>" + data[i].name + "</option>");
                    if(data[i].id in optgroups) {
                        optgroups[data[i].id].append($el);
                        $select.append(optgroups[data[i].id]);
                    } else {
                        $select.append($el);
                    }
                }
    ...
    function createVariableView(groupId, memberId, data, selectedValue) {
        var $el = $("<span aeType='aeCombo' id='var_"+memberId+"' aeInit='false' class='none' width='150px' optionField='name' valueField='id' data-role='member-variable' data-group-id='" + groupId + "' data-member-id='" + memberId + "'>");
        return $el;
    }

    function createOperatorView(groupId, memberId, data, selectedValue) {
        var $el = $("<span aeType='aeCombo' id='opr_"+memberId+"' class='none' width='160px' optionField='name' valueField='code' data-role='member-operator' data-value-type='"+data.valueType+"' data-group-id='" + groupId + "' data-member-id='" + memberId + "'>");
        return $el;
    }

    function hdDateChange(target) {
        var $el = target.parent().prev(),
            groupId = $el.attr("data-group-id"),
            memberId = $el.attr("data-member-id"),
            valueType = $el.attr("data-value-type"),
            obj = {
                "groupId": groupId,
                "memberId": memberId,
                "valueType": parseInt(valueType,10)
            };
        updateExpression(obj);
    }
    window.hdDateChange = hdDateChange;

    function createValueView($cntr,groupId, memberId, valueType,val, defaultValue) {
        var $els,
            $el,
            $elFlip,
            opt,
            s="<span aeType='aeCombo' id='enum_"+memberId+"' aeInit='false' class='none' width='150px' optionField='name' valueField='code' data-role='member-value-enum' data-value-type='"+valueType+"' data-group-id='" + groupId + "' data-member-id='" + memberId + "'/><input type='text' class='form-control' data-role='member-value' data-value-type='"+valueType+"' data-group-id='" + groupId + "' data-member-id='" + memberId + "' style='width:150px'/><span aeType='aeCalendar' width='150px' data-role='member-value-date' data-value-type='"+valueType+"' data-group-id='" + groupId + "' data-member-id='" + memberId + "' onValueChange='hdDateChange'></span><span aeType='aeCalendar' width='150px' data-role='member-value-datetime' data-value-type='"+valueType+"' data-group-id='" + groupId + "' data-member-id='" + memberId + "' onValueChange='hdDateChange'></span><span aeType='aeFlip' data-role='member-value-number' width='150px' minData='0' data-value-type='"+valueType+"' data-group-id='" + groupId + "' data-member-id='" + memberId + "'></span>";

        $els = $(s);
        $cntr.append($els);
        $el=$cntr.find("[data-role='member-value-enum']");
        $el.aeCombo({
            visible:false
        });
        ...
        $cntr.find("[data-role='member-value-datetime']").aeCalendar({
            showTime: true,
            visible: false
        });
        $elFlip=$cntr.find("[data-role='member-value-number']");
        $elFlip.aeFlip({
            visible: false
        });
        ...
        if($.isArray(val)) {
            $el = $($els[0]);
            $el.aeCombo("visible",true);
            $el.next().addClass("m-b-xs");
        } else {
            if(valueType == 2) { //数字类型
                $el = $cntr.find("[data-role='member-value-number']");
                $el.aeFlip("visible",true);
                $el.aeFlip("setValue", (val || defaultValue || ""));
            }else if(valueType == 3) { //日期类型
                $el = $cntr.find("[data-role='member-value-date']");
                $el.aeCalendar("visible", true);
                $el.next().addClass("m-b-xs");
                $el.aeCalendar("setValue", (val || defaultValue || ""));
            }else if(valueType == 4) { //日期时间类型
                $el = $cntr.find("[data-role='member-value-datetime']");
                $el.aeCalendar("visible", true);
                $el.next().addClass("m-b-xs");
                $el.aeCalendar("setValue", (val || defaultValue || ""));
            } else {
                $el = $($els[1]);
                $el.addClass("m-b-xs");
                $el.val(val || defaultValue || "");
                $el.show();
            }
        }

        $elFlip.aeFlip("option",opt);
        return $els;
    }

    function updateExpressionWithCalendar($member) {
        var memberId = $member.attr("id"),
            groupId,
            condi,
            member,
            exprId;
        groupId = $member.attr("data-group-id");
        member = ExpressionHelper.getGroup(groupId).getMember(memberId);
        condi = getCondition(memberId, 3);
        member.value = condi.value;
        member.valueText = condi.valueText;
        member.valueType = condi.valueType;
        member.operator = condi.operator;
        member.operatorName = condi.operatorName;
        member.controlType = condi.controlType;
        member.variable = condi.variable;
        member.variableId = condi.variableId;
        member.variableName = condi.variableName;
        member.template = condi.template;
        member.displayTemplate = condi.displayTemplate;
        //触发改变事件
        exprId = $("#" + groupId).attr("data-expr-id");
        ExpressionHelper.fireExprChangeEvent(exprId);
    }

    function updateExpression(ownerInfo) {
        var memberId,
            groupId,
            condi,
            member,
        ...
        if($input) {
            ...
            memberId = $input.attr("data-member-id");
            valueType = parseInt($input.attr("data-value-type"),10);
        } else {
            groupId = ownerInfo.groupId;
            memberId = ownerInfo.memberId;
            valueType = ownerInfo.valueType;
        }
        member = ExpressionHelper.getGroup(groupId).getMember(memberId);
        condi = getCondition(memberId, valueType);
        member.value = condi.value;
        member.valueText = condi.valueText;
        member.valueType = condi.valueType;
        member.operator = condi.operator;
    ...
    function refreshOperators(ownerInfo, variablesData,defaultValue) {
        var memberId = ownerInfo.memberId,
            $member = $("#" + memberId),
            $elOpers = $member.find("[data-role='member-operator']");

        // $elOpers.val(null).empty();
        // addOperatorItems($elOpers, variablesData.operators);
        // $elOpers.val(defaultValue || $elOpers.children(":first").val()).trigger("change");
        $elOpers.aeCombo("reload",variablesData.operators);
        $elOpers.aeCombo("setDefault",(defaultValue || variablesData.operators[0].code));
        setOperatorsUserdata($elOpers,variablesData.operators);
        _operatorChange($elOpers);
    }

    function updateValuesModel(ownerInfo) {
        var memberId = ownerInfo.memberId,
            $member = $("#" + memberId),
            controlType=ownerInfo.controlType,
            valueType=ownerInfo.valueType,
            exprId;

        if(controlType == CTRL_NUM) {
            updateExpression(ownerInfo);
        }else if(controlType == CTRL_TEXT) {
            updateExpression(ownerInfo);
        }else if(controlType == CTRL_DATE) {
            if(valueType==VALUE_TYPE_DATE){
                updateExpressionWithCalendar($member);
            }else if(valueType==VALUE_TYPE_DATETIME){
                updateExpressionWithCalendar($member);
            }
            ...
            updateExpression(ownerInfo);
            //触发改变事件
            // exprId = $("#" + ownerInfo.groupId).attr("data-expr-id");
            // ExpressionHelper.fireExprChangeEvent(exprId);
        }
    }

    function refreshValues(ownerInfo,optionValue, defaultValue) {
        var memberId = ownerInfo.memberId,
            $member = $("#" + memberId),
            groupId = ownerInfo.groupId,
            $elEnum = $member.find("[data-role='member-value-enum']"),
            $elValue = $member.find("[data-role='member-value']"),
            $elDate = $member.find("[data-role='member-value-date']"),
            $elDateTime = $member.find("[data-role='member-value-datetime']"),
            $elNumberValue = $member.find("[data-role='member-value-number']"),
            valueType = ownerInfo.valueType,
            controlType=ownerInfo.controlType,
            finalValue;

        $elValue.hide();
        $elEnum.aeCombo("visible", false);
        $elDate.aeCalendar("visible", false);
        $elDateTime.aeCalendar("visible", false);
        $elNumberValue.aeFlip("visible",false);
        if(defaultValue==="?"){
            defaultValue="";
        }
        finalValue=(optionValue || defaultValue);
        if(controlType == CTRL_NUM) {
            if($.isArray(finalValue)){
                finalValue="";
            }
            $elNumberValue.aeFlip("setValue",finalValue);
            $elNumberValue.aeFlip("visible",true);
        }else if(controlType == CTRL_TEXT) {
            if($.isArray(finalValue)){
                finalValue="";
            }
            $elValue.val(finalValue);
            $elValue.show();
        }else if(controlType == CTRL_DATE) {
            if($.isArray(finalValue)){
                finalValue="";
            ...
            if(valueType==VALUE_TYPE_DATE){
                $elDate.aeCalendar("setValue", finalValue);
                $elDate.aeCalendar("visible", true);
            }else if(valueType==VALUE_TYPE_DATETIME){
                $elDateTime.aeCalendar("setValue", finalValue);
    ...
    function refreshMemberView(nodeData){
        var memberId = window.logicblock_share_data.memberId,
            groupId = window.logicblock_share_data.groupId,
            $member = $("#" + memberId),
            $g = $("#" + groupId),
            $btn = $member.find("[data-role='member-tree-button']");
        delete window.logicblock_share_data;
        $btn.attr("data-code", nodeData.code).attr("data-variable-id", nodeData.id).text(nodeData.name);
        $member.find("[data-role='member-operator']").attr("data-value-type", nodeData.valueType);
        $member.find("[data-role='member-value']").attr("data-value-type", nodeData.valueType);
        $member.find("[data-role='member-value-enum']").attr("data-value-type", nodeData.valueType);
        $member.find("[data-role='member-value-date']").attr("data-value-type", nodeData.valueType).attr("data-group-id", groupId).attr("data-member-id", memberId);
        $member.find("[data-role='member-value-datetime']").attr("data-value-type", nodeData.valueType).attr("data-group-id", groupId).attr("data-member-id", memberId);
        $member.find("[data-role='member-value-number']").attr("data-value-type", nodeData.valueType).attr("data-group-id", groupId).attr("data-member-id", memberId);
        refreshOperators({
            "groupId": groupId,
            "memberId": memberId
        }, nodeData);
    ...
    function hdVariablesTreeSelect(nodeData) {
        if(nodeData.valueType !== undefined) {
            $.closePopupDiv();
            refreshMemberView(nodeData);
        }
    }
    window.hdVariablesTreeSelect = hdVariablesTreeSelect;

    function hdTreeButtonClick(event) {
    ...
    function getFirstVariableObject(variables) {
        ...
        for(i = 0; i < count; i++) {
            if(typeof(variables[i].valueType)==="number" && isNaN(variables[i].valueType)===false) {
                return variables[i];
            }
        }
        return null;
    ...
    function hdDelGroup(event){
        var groupId=$(this).attr("data-group-id");

        ExpressionHelper.delGroup(groupId);
        $("#"+groupId).fadeOut(400, function() {
            var $this=$(this);
            $this.remove();
            //触发改变事件
            exprId = $this.attr("data-expr-id");
            ExpressionHelper.fireExprChangeEvent(exprId);
        });
    }

    function getValidVariables(variables){
        var i,
            count,
            arr=[];
    ...
    function setVariablesUserData($el,variables) {
        var i,
            j=0,
            count,
            idxs,
        ...
        if($.isArray(variables)){
            ...
            for(i=0;i<count;i++){
                if(typeof(variables[i].valueType)==="number" && isNaN(variables[i].valueType)===false) {
                    idxs.push(j++);
                    datas.push(variables[i]);
                }
            }
            $el.aeCombo("setUserdata",idxs,datas);
        }
    }

    function setOperatorsUserdata($el,operators){
        var i,
            count,
            idxs,
            datas;
        if($.isArray(operators)){
            ...
            for(i=0;i<count;i++){
                idxs.push(i);
                datas.push(operators[i]);
            }
            $el.aeCombo("setUserdata",idxs,datas);
        }
    }

    function hdAddMember(event) {
        var $this = $(this),
            memberId = getId("mid"),
            $memberView,
            $cntr,
        ...
        if(option.hasCatalog) {
            variableObject = getFirstVariableObject(option.variables);
            if(variableObject !== null) { //数据没有异常
                $btn = createVariableTreeButtonView(groupId, memberId, option.variables);
                $btn.attr("data-code", variableObject.code).attr("data-variable-id", variableObject.id).text(variableObject.name);
                $cntr.append($btn);
                $btn.on("click.aeLogicBlock", hdTreeButtonClick);
            }
        } else {
            $select = createVariableView(groupId, memberId, option.variables);
            $cntr.append($select);
            $select.aeCombo({});
            $select.next().addClass("m-b-xs");
            $select.aeCombo("reload",getValidVariables(option.variables));
            setVariablesUserData($select,option.variables);
            variableObject = $select.aeCombo("getUserdata",0);
            $select.aeCombo("setDefault",variableObject.id);
        }
        operators=variableObject.operators;
        tmpl=operators[0].template;
        ...
        $select.aeCombo("setDefault",operators[0].code);
        setOperatorsUserdata($select,operators);
        $select = createValueView($cntr,groupId, memberId, variableObject.valueType,variableObject.value,defaultValue);
        ...
        expr.getGroup(groupId).addMember(member);
        //触发改变事件
        ExpressionHelper.fireExprChangeEvent(exprId);
    }

    function hdDelMember(event) {
        var $this = $(this),
            memberId = $this.attr("data-member-id");
        $("#" + memberId).fadeOut(400, function() {
            var $this = $(this),
                group,
                memberId = $this.attr("id"),
                groupId = $this.attr("data-group-id");
            group = ExpressionHelper.getGroup(groupId);
            group.removeMember(group.getMember(memberId));
            $this.remove();
            delGroupView(groupId);
            //触发改变事件
            exprId = $("#" + groupId).attr("data-expr-id");
            ExpressionHelper.fireExprChangeEvent(exprId);
        });
    ...
    function delGroupView(groupId) {
        var $g = $("#" + groupId),
            isRootGroup = ($g.attr("data-root-group") === "1"),
            $cntr,
            parentGroupId,
            exprId;
        if(isRootGroup) {
        ...
        $cntr = $g.children("[data-role='group-cntr']");
        parentGroupId = $g.parent().attr("data-group-id");
        exprId = $g.attr("data-expr-id");
        if($cntr.children("[data-role='member']").length === 0 && $cntr.children("[data-role='member-group']").length === 0) { //没有成员和组,即组已空,那么干掉此组和此组的祖先空组
            ExpressionHelper.delGroup(groupId);
            $g.fadeOut(400, function() {
                $(this).remove();
                ExpressionHelper.fireExprChangeEvent(exprId);
                delGroupView(parentGroupId);
            });
        }
    }

    function hdEnumValueChange(event) {
        var $this = $(this).parent().prev();
        _enumValueChange($this);
    }

    function _enumValueChange($span) {
        var memberId,
            groupId,
            condi,
            member,
            exprId;
        groupId = $span.attr("data-group-id");
        memberId = $span.attr("data-member-id");
        member = ExpressionHelper.getGroup(groupId).getMember(memberId);
        condi = getCondition(memberId, 0);
        member.value = condi.value;
        member.valueText = condi.valueText;
        member.valueType = condi.valueType;
        member.operator = condi.operator;
    ...
    function hdVariableChange(event) {
        var $this = $(this).parent().prev(),
            groupId = $this.attr("data-group-id"),
            $g = $("#" + groupId),
            exprId = $g.attr("data-expr-id"),
        ...
        $member.find("[data-role='member-value']").attr("data-value-type", valueType);
        $member.find("[data-role='member-value-enum']").attr("data-value-type", valueType);
        $member.find("[data-role='member-value-date']").attr("data-value-type", valueType).attr("data-group-id", groupId).attr("data-member-id", memberId);
        $member.find("[data-role='member-value-datetime']").attr("data-value-type", valueType).attr("data-group-id", groupId).attr("data-member-id", memberId);
        $member.find("[data-role='member-value-number']").attr("data-value-type", valueType).attr("data-group-id", groupId).attr("data-member-id", memberId);
        refreshOperators({
            "groupId": groupId,
            "memberId": memberId
        }, option.variables[$this.aeCombo("getIndex")]);
    }

    function getOptionValue($member,option,groupId) {
        var variableId,
            mapVars=getMapVars(groupId);
        if(option.hasCatalog) {
            $variable = $member.find("[data-role='member-tree-button']");
            variableId=$variable.attr("data-variable-id");
        } else {
            $variable = $member.find("[data-role='member-variable']");
            idx = $variable.aeCombo("getIndex");
            variableId = option.variables[idx].id;
        }
        return mapVars[variableId].value;
    }

    function getDefaultValue($member) {
        var $el,
            defaultValue,
            ud;
        $el=$member.find("[data-role='member-operator']");
        ud=$el.aeCombo("getUserdata",$el.aeCombo("getIndex"));
        defaultValue=ud.defaultValue;
        return defaultValue;
    }

    function hdOperatorChange(event) {
        var $this = $(this).parent().prev();
        _operatorChange($this);
    }
    function _operatorChange($span) {
        var groupId = $span.attr("data-group-id"),
            memberId = $span.attr("data-member-id"),
            $g = $("#" + groupId),
            $member=$("#"+memberId),
            exprId = $g.attr("data-expr-id"),
            ...
            optionValue=getOptionValue($member,option,groupId),
            ownerInfo;

        valueType = $span.attr("data-value-type");
        controlType=parseInt($span.aeCombo("getUserdata",$span.aeCombo("getIndex")).controlType,10);
        ownerInfo = {
            groupId: groupId,
            memberId: memberId,
            valueType: parseInt(valueType,10),
        ...
        refreshValues(ownerInfo,optionValue,defaultValue);
        updateValuesModel(ownerInfo);
    }

    function hdValueChange(event) {
        if(hdTimer !== null) {
            clearTimeout(hdTimer);
            hdTimer = null;
        }
        $input = $(this);
        hdTimer = setTimeout(updateExpression, 500);
    }

    function getNextOper(logicOperators, currentOperator) {
        var i,
            count,
            item;
        if($.isArray(logicOperators)) {
            count = logicOperators.length;
            for(i = 0; i < count; i++) {
                item = logicOperators[i];
                if(item.code === currentOperator) {
                    return {
                        current: logicOperators[i],
                        next: logicOperators[(i >= count - 1) ? 0 : (i + 1)]
                    };
                }
            }
        }
    ...
    function hdChangeOper(event) {
        var $this = $(this),
            exprId,
            operator = $this.attr("data-oper"),
            groupId = $this.attr("data-group-id"),
            $g = $("#" + groupId),
            group = ExpressionHelper.getGroup(groupId),
        ...
        if(logics !== null) {
            ...
            $this.attr("data-oper", logics.next.code).text(logics.next.text);
            group.operatorName = logics.next.text;
            group.operator = logics.next.code;
            //触发改变事件
            exprId = $g.attr("data-expr-id");
            ExpressionHelper.fireExprChangeEvent(exprId);
        }
    ...
    function hdStartDrag(event, ui) {
        var $this = $(this),
            ...
            $tips;
        disableMemberDroppable(exprId);
        $expr.find("[data-role='member-icon']").not("[data-member-id='" + $this.attr("id") + "']").removeClass("ion-drag").addClass("ion-arrow-shrink");
        $expr.find("[data-role='member-del']").hide();
        $btns = $expr.find("[data-role='member-add']");
        $btns.not("[data-group-id='" + groupId + "']").hide();
        $tips = $expr.find("[data-role='member-drop-tip']");
        $tips.not("[data-group-id='" + groupId + "']").show();
        $this.css("z-index", 9999);
    }

    function hdStartGroupDrag(event, ui) {
        var $this = $(this),
        ...
        $g.find("[data-role='member-drop-tip']").attr("data-excluder", "1");
        $btns = $expr.find("[data-role='member-add']");
        $btns.not("[data-excluder='1']").hide();
        $tips = $expr.find("[data-role='member-drop-tip']");
        $tips.not("[data-excluder='1']").show();
        $parentGroupCntr.children("[data-role='member-add']").show();
        $parentGroupCntr.children("[data-role='member-drop-tip']").hide();
        $this.css("z-index", 9999);
    }

    function hdStopDrag(event, ui) {
        var $this = $(this),
            groupId = $this.attr("data-group-id"),
            exprId = $("#" + groupId).attr("data-expr-id"),
            $expr = $('[data-expr-inst-id="'+exprId+'"]');
        ...
        $expr.find("[data-role='member-add']").show();
        $expr.find("[data-role='member-drop-tip']").hide();
        $this.css({"z-index":0,"width":"100%"});
    }

    function hdStopGroupDrag(event, ui) {
        var $this = $(this),
            groupId = $this.attr("id"),
            $g = $("#" + groupId),
            exprId = $g.attr("data-expr-id"),
        ...
        $expr.find("[data-role='member-del']").show();
        $btns = $expr.find("[data-role='member-add']");
        $btns.not("[data-excluder='1']").show();
        $tips = $expr.find("[data-role='member-drop-tip']");
        $tips.not("[data-excluder='1']").hide();
        $g.find("[data-role='member-add']").attr("data-excluder", "0");
        $g.find("[data-role='member-drop-tip']").attr("data-excluder", "0");
        $this.css({"z-index":0,"width":"100%"});
    }

    function hdOver(event, ui) {
        var $this = $(this),
            dataRole = $this.attr("data-role"),
            groupId;
        if(dataRole === "member-drop-tip") {
            //groupId=$this.attr("data-group-id");
            //$("#"+groupId).addClass("b-danger");
            $this.addClass("b-danger");
        } else if(dataRole === "member") {
            $this.addClass("b-danger");
        }
    }

    function hdOut(event, ui) {
        var $this = $(this),
            dataRole = $this.attr("data-role"),
            groupId;
        if(dataRole === "member-drop-tip") {
            //groupId=$this.attr("data-group-id");
            //$("#"+groupId).removeClass("b-danger");
    ...
    function hdDrop(event, ui) {
        var $this = $(this),
            $btn,
            draggable = event.draggable,
            thisRole = $this.attr("data-role"),
            newGroupId,
            exprId,
            groupId,
            groupView,
        ...
        if(thisRole === "member") { //合并两个条件为新的组,此组加入到目标条件所在的组内
            ...
            groupView.attr("data-expr-id", $("#" + parentGroupId).attr("data-expr-id")); //继承父组上的exprId
            groupView.insertBefore($this);
            $el = $("#" + newGroupId);
            $btn = $el.find("[data-role='group-cntr']").children("[data-role='member-add']");
            $this.insertBefore($btn);
            draggable.insertBefore($btn);
            $this.attr("data-group-id", newGroupId);
            $this.find("[data-group-id]").attr("data-group-id", newGroupId);
            draggable.attr("data-group-id", newGroupId);
            draggable.find("[data-group-id]").attr("data-group-id", newGroupId);

            $el.attr("data-expr-id", exprId);
            //生成模型
            group = ExpressionHelper.addGroup(newGroupId, parentGroupId);
            member = ExpressionHelper.getGroup(parentGroupId).getMember($this.attr("id"));
            ...
            if(draggable.attr("data-role") === "member-group") { //组加入组
                ...
                draggable.insertBefore($btn);
                //更新模型
                ExpressionHelper.getGroup(groupId).addMember(group);
            } else {
                groupId = draggable.attr("data-group-id");
                prevGroupId = groupId;
                groupId = $this.attr("data-group-id");
                ...
                $btn = $el.find("[data-role='member-add']").filter("[data-group-id='" + groupId + "']");
                draggable.insertBefore($btn);
                draggable.attr("data-group-id", groupId);
                draggable.find("[data-group-id]").attr("data-group-id", groupId);
                //更新模型
                member = ExpressionHelper.getGroup(prevGroupId).getMember(draggable.attr("id"));
                ExpressionHelper.getGroup(groupId).addMember(member);
            }
        }
        if(prevGroupId) {
            delGroupView(prevGroupId);
        }
        //触发改变事件
        ExpressionHelper.fireExprChangeEvent(exprId);
        return false;
    ...
    function addGroupEvents($cntr) {
        $cntr.on("click.aeLogicBlock", "[data-role='member-add']", hdAddMember).on("click.aeLogicBlock", "[data-role='group-del']", hdDelGroup).on("click.aeLogicBlock", "[data-role='oper']", hdChangeOper).on("mouseover.aeLogicBlock", "[data-role='drag-handle'],[data-role='group-del'],[data-role='member-add']", hdNotOpacity).on("mouseout.aeLogicBlock", "[data-role='drag-handle'],[data-role='group-del'],[data-role='member-add']", hdHalfOpacity);
    }

    function removeGroupEvents($cntr) {
        $cntr.off("click.aeLogicBlock").off("mouseover.aeLogicBlock").off("mouseout.aeLogicBlock");
    }

    function addMemberEvents($memberView) {
        var inited = $memberView.data("inited");
        $memberView.on("click.aeLogicBlock", "[data-role='member-del']", hdDelMember);
        $memberView.find("[data-role='member-variable']").aeCombo("option","onValueChange",hdVariableChange);
        $memberView.find("[data-role='member-operator']").aeCombo("option","onValueChange",hdOperatorChange);
        $memberView.find("[data-role='member-value']").on("keyup.aeLogicBlock", hdValueChange);
        $memberView.find("[data-role='member-value-enum']").aeCombo("option","onValueChange",hdEnumValueChange);
        if(inited) {
            $memberView.aeDraggable("enable").aeDroppable("enable");
        } else {
            $memberView.data("inited", true);
            $memberView.aeDraggable({
                handle: "[data-role='drag-handle']",
                revert: true,
                distance: 5,
                start: hdStartDrag,
        ...
        $memberView.find("[data-role='member-icon'],[data-role='member-del']").on("mouseover.aeLogicBlock", hdNotOpacity).on("mouseout.aeLogicBlock", hdHalfOpacity);
    }

    function removeMemberEvents($memberView) {
        $memberView.off("click.aeLogicBlock");
        $memberView.find("[data-role='member-variable']").off("change.aeLogicBlock");
        $memberView.find("[data-role='member-operator']").off("change.aeLogicBlock");
        $memberView.find("[data-role='member-value']").off("keyup.aeLogicBlock");
        $memberView.find("[data-role='member-del']").off("mouseover.aeLogicBlock").off("mouseout.aeLogicBlock");
        $memberView.aeDraggable("disable").aeDroppable("disable");
    }
    /**
     * 表达式可视化编辑组件
     * @namespace ae.aeLogicBlock
     */
    $.aeWidget("ae.aeLogicBlock", {
         ...
         *         operatorCode=detailedList.operatorCode,
         *         operatorName=detailedList.operatorName,
         *         value=detailedList.value,
         *         valueType=detailedList.valueType,
         *         template=detailedList.template,
         *         displayTemplate=detailedList.displayTemplate,
         *         templateParser=detailedList.templateParser,
         *         userData=detailedList.userData,
         *         exprStr;
         *
         ...
         *         }else{
         *             exprStr=templateParser(variableCode,value,template);
         *         }
         ...
         *         }
         *     ],
         *     assembler:function(detailedList) {
         *         var variableCode=detailedList.variableCode,
         *             variableName=detailedList.variableName,
         *             operatorCode=detailedList.operatorCode,
         *             operatorName=detailedList.operatorName,
         *             value=detailedList.value,
         *             valueText=detailedList.valueText,
         *             valueType=detailedList.valueType,
         *             template=detailedList.template,
         *             displayTemplate=detailedList.displayTemplate,
         *             templateParser=detailedList.templateParser,
        ...
        options: {
            ...
            onExprChange: $.noop,
            assembler: null,
            templateParser:defaultTemplateParser
        },
        _create: function() {
            var $el = this.element,
        ...
        _initSearchbar:function(){
            ...
            if(!this._$tree){
                ...
                this._$result.on("mouseover.aeLogicBlock","li",this,function(event){
                    $(this).addClass(CSS_HIGHLIGHT);
                }).on("mouseout.aeLogicBlock","li",this,function(event){
                    $(this).removeClass(CSS_HIGHLIGHT);
                }).on("click.aeLogicBlock","li",this,function(event){
                    var inst=event.data,
                        nodeId=$(this).attr("data-node-id");
                    inst._gotoNode(nodeId);
                });
                this._$input=$dlg.find('[data-role="search-input"]');
                this._$input.on("keyup.aeLogicBlock",this,function(event){
                    var inst=event.data,
                        selector=".bg-light",
                        $el;

                    if(inst._hdTimer!==null){
                        clearTimeout(inst._hdTimer);
                        inst._hdTimer=null;
                    }
                    switch(event.which){
                        case 27:
                            $(this).trigger("blur.aeLogicBlock",inst);
                            return false;
                        case 13:
                            if(inst._nodeId){
                                inst._gotoNode(inst._nodeId);
                            }
                            break;
                        case 38: //up
                            $el=inst._$result.find("li").eq(0);
                            if(!(inst._nodeId && inst._nodeId===$el.attr("data-node-id"))){
                                $el=inst._$result.find(selector);
                                if($el.length>0){
                                    $el=$el.removeClass(CSS_HIGHLIGHT).prev().addClass(CSS_HIGHLIGHT);
                                    inst._nodeId=$el.attr("data-node-id");
                                }
                            }
                            return false;
                        case 40: //down
                            $el=inst._$result.find("li").filter(":last");
                            if(!(inst._nodeId && inst._nodeId===$el.attr("data-node-id"))){
                                $el=inst._$result.find(selector);
                                if($el.length===0){
                                    $el=inst._$result.find("li").eq(0).addClass(CSS_HIGHLIGHT);
                                }else{
                                    $el=$el.removeClass(CSS_HIGHLIGHT).next().addClass(CSS_HIGHLIGHT);
                                }
                                inst._nodeId=$el.attr("data-node-id");
                            }
                            return false;
                    }
                    inst._$input.parent().find('[data-role="search-clear"]').show();
                    inst._hdTimer=setTimeout($.proxy(inst._refreshResults,inst),300);
                }).parent().find('[data-role="search-clear"]').on("click.aeLogicBlock",this,function(event){
                    event.data._resetDialog();
                });
            }
        },
        _resetDialog:function(){
            this._nodeId=null;
            this._$input.parent().find('[data-role="search-clear"]').hide();
            this._$input.val("");
            this._$result.hide();
            this._$tree.show();
        },
        _gotoNode:function(nodeId){
            var $tree=this._$tree.find('[aeType="aeTree"]');
            $tree.aeTree("expandNode",nodeId);
            $tree.aeTree("selectNode",nodeId);
            this._$result.hide();
            this._$tree.show();
        },
        _refreshResults:function(){
            var value,
                nodeIds,
                $notFound=this._$result.find('[data-role="not-found"]');

            $notFound.hide().siblings().remove();
            ...
            if(value===""){
                ...
                if(value.length>=1){
                    nodeIds=this._search(value);
                    if(nodeIds.length>0){
                        this._reloadResult(nodeIds);
                    }else{
                        $notFound.show();
                    }
                }
        ...
        _search:function(condition){
            var exprId=this.element.data("expr").id, //this.element.attr("id")
                ...
                item,
                arr=[],
                lowerCase=condition.toLowerCase();
            if($el.length>0){
                cache=$el.data("nodes");
                for(nodeId in cache){
                    item=cache[nodeId];
                    if(item && item.valueType!==undefined && typeof(item.name)==="string"){
                        if(item.name.toLowerCase().indexOf(lowerCase)>=0){
                            arr.push({
                                id:nodeId,
                                info:item
                            });
        ...
        _reloadResult:function(nodeInfos){
            var i,
                count=nodeInfos.length,
                s="";
            for(i=0;i<count;i++){
                s+=getResultItemString(nodeInfos[i].id,nodeInfos[i].info);
            }
        ...
        _preprocessVariables: function() {
            ...
            for(i = variables.length-1; i >=0; i--) {
                if($.isArray(variables[i].operators)){
                    variables[i].value=variables[i].value || null;
                    variables[i].valueType=parseInt(variables[i].valueType,10);
                    ...
                    for(j = operators.length-j; j >=0; j--) {
                        ...
                        if(typeof(oper.defaultValue)!=="string"){
                            oper.defaultValue=null;
                        }
                    }
                    mapVars[variables[i].id]=variables[i];
                }
            }
            this.element.data("map_vars",mapVars);
        },
        _initModel: function(exprId, groupId) {
            var expr,
        ...
        _initView: function(exprId, groupId) {
            ...
            $g.find("[data-role='group-del']").remove();
            $g.find("[data-role='member-drop-tip']").aeDroppable({
                greedy: true,
                drop: hdDrop,
                over: hdOver,
                out: hdOut
            ...
            if(this.options.hasCatalog) {
                this._initVariableTreeDialog(exprId);
            }
            return $g;
        },
        _initVariableTreeDialog: function(exprId) {
            var $dlg,
                $tree;
            $dlg = createVariableTreeDialogView(exprId, this.options.variables);
            $tree = $dlg.find("[data-role='member-tree']");
            $tree.aeTree({});
            $tree.aeTree("remove");
            $tree.aeTree("reload", this.options.variables);
        },
        /**
         * 启用组件
         * @name ae.aeLogicBlock#enable
         * @function
         */
        enable: function() {
            var el=this.element,
                $members = el.find("[data-role='member']"),
                i;
            addGroupEvents(el);
            el.find("[data-role='member-group']").not("[data-root-group]").aeDraggable("enable");
            el.find("[data-role='oper']").css("cursor", "hand");
            el.find("[data-role='group-del']").show();
            el.find("[data-role='drag-handle']").show();
            el.find("[data-role='member-variable']").aeCombo("enable", true);
            el.find("[data-role='member-operator']").aeCombo("enable", true);
            el.find("[data-role='member-value']").attr("disabled", false);
            el.find("[data-role='member-value-enum']").aeCombo("enable", true);
            el.find("[data-role='member-value-number']").aeFlip("enable", true);
            el.find("[data-role='member-tree-button']").attr("disabled", false);
            el.find("[data-role='member-del']").show();
            el.find("[data-role='member-add']").show();
            for(i = $members.length - 1; i >= 0; i--) {
                addMemberEvents($($members[i]));
            }
        },
        /**
         * 禁用组件
         * @name ae.aeLogicBlock#disable
         * @function
         */
        disable: function() {
            var el=this.element,
                $members = el.find("[data-role='member']"),
                i;
            removeGroupEvents(el);
            el.find("[data-role='member-group']").not("[data-root-group]").aeDraggable("disable");
            el.find("[data-role='oper']").css({
                "cursor": "default"
            });
            el.find("[data-role='group-del']").hide();
            el.find("[data-role='drag-handle']").hide();
            el.find("[data-role='member-variable']").aeCombo("enable", false);
            el.find("[data-role='member-operator']").aeCombo("enable", false);
            el.find("[data-role='member-value']").attr("disabled", true);
            el.find("[data-role='member-value-enum']").aeCombo("enable", false);
            el.find("[data-role='member-value-number']").aeFlip("enable", false);
            el.find("[data-role='member-tree-button']").attr("disabled", true);
            el.find("[data-role='member-del']").hide();
            el.find("[data-role='member-add']").hide();
            for(i = $members.length - 1; i >= 0; i--) {
                removeMemberEvents($($members[i]));
            }
        },
        /**
         * 导出表达��?
         ...
         * @function
         * @private
         * @return {string} 返回被清理掉掉Expression实例掉exprId
         */
        _clear: function() {
            var expr = this.element.data("expr"),
                exprId = expr.id;
        ...
        _getVariableObject: function(variables, vairableName) {
            var i;
            for(i = variables.length - 1; i >= 0; i--) {
                if(variables[i].name === vairableName) {
                    return variables[i];
                }
            }
            return null;
        ...
        _getLogicOperator: function(operCode) {
            var logicOperators = this.options.logicOperators,
                i;
            for(i = logicOperators.length - 1; i >= 0; i--) {
                if(logicOperators[i].code === operCode) {
         ...
         * 重新装载视图
         * @ignore
         * @name ae.aeLogicBlock#_reloadViews
         * @function
         * @private
         * @param $parentGroupView 父组DOM元素实例
         * @param {array} members 包含了多个Member或MemberGroup类实例的数组
         */
        _reloadViews: function($parentGroupView, members) {
            var i,
                count,
                $btn,
                $oper,
            ...
            for(i = 0; i < count; i++) {
                if(members[i] instanceof MemberGroup) {
                    ...
                    logicOperator = this._getLogicOperator(members[i].operator);
                    if(logicOperator !== null) {
                        $oper = $groupView.find("[data-role='oper']");
                        $oper.attr("data-oper", logicOperator.code).text(logicOperator.text);
                        groupId = $oper.attr("data-group-id");
                        $("#" + groupId).addClass(logicOperator.color);
                    }
                    $groupView.find("[data-role='member-drop-tip']").aeDroppable({
                        greedy: true,
                        drop: hdDrop,
                        over: hdOver,
                        out: hdOut
                    });
                    $groupView.aeDraggable({
                        handle: "[data-role='drag-handle']",
                        revert: true,
                        distance: 5,
                        start: hdStartGroupDrag,
                    ...
                    this._reloadViews($groupView, members[i].members);
                } else {
                    groupId = $parentGroupView.attr("id");
                    ...
                    if(this.options.hasCatalog) {
                        $btnSelectTree = createVariableTreeButtonView(groupId, memberId, options.variables);
                        $btnSelectTree.attr("data-code", members[i].variable).attr("data-variable-id", members[i].variableId).text(members[i].variableName);
                        $cntr.append($btnSelectTree);
                        $btnSelectTree.on("click.aeLogicBlock", hdTreeButtonClick);
                    } else {
                        $select = createVariableView(groupId, memberId, options.variables, variableObject.name);
                        $cntr.append($select);
                        $select.aeCombo({});
                        $select.next().addClass("m-b-xs");
                        $select.aeCombo("reload",options.variables);
                        $select.aeCombo("setDefault",options.variables[0].id);
                        setVariablesUserData($select,options.variables);
                    }
                    ...
                    $elOpers.aeCombo("setValue",members[i].operator);
                    $elOpers.aeCombo("option","onValueChange",optValueChange); //恢复回调函数

                    ownerInfo = {
                        groupId: groupId,
                        memberId: memberId,
                        valueType: parseInt(members[i].valueType,10),
        ...
        _reloadRoot:function($root,state){
            var $glbl,
                ...
                item,
                operator=null;
            $glbl=$root.find("[data-role='oper']");
            for(i=groups.length-1;i>=0;i--){
                if(groups[i].id===state.rootId){
                    operator=groups[i].operator;
                }
            }
            if(operator!==null){
                item=this._getLogicOperator(operator);
                $glbl.attr("data-oper",item.code).text(item.text);
                $("#" + $root.attr("id")).addClass(item.color);
            }
        },
        /**
         * 重新加载数据
        ...
        reload: function(state) {
            ...
            ExpressionHelper.fireExprChangeEvent(state.id);
        },
        _destroy:function(){
            ...
            this._$input.off(".aeLogicBlock");
            this._$input.remove();
            this._$input=null;
            this._$result.remove();
            this._$result=null;
            this._clear();
        }
    });
...
/**
 * Created by Harwin on 2015/12/21.
 */
define('ui-notice', function (require, exports, moudles) {
    ...
    $.aeWidget("ae.aeNotice", {
        ...
        _create: function () {
            ...
            for (i = 0; i <= 5; i++) {
                ...
                containers[i].attr('id', pos[i].h + '' + pos[i].v);
                containers[i].addClass('notice-container' + ' ' + pos[i].h + ' ' + pos[i].v);
                this.element.append(containers[i]);
            }
        ...
        _createNotice: function (options) {
            var i,
                ...
                pos,
                container,
                v = options.verticalEdge;
            h = options.horizontalEdge;
            pos = {h: h, v: v};
            for (i = 0; i < this.pos.length; i++) {
                if (pos.h === this.pos[i].h && pos.v === this.pos[i].v) {
                    container = this.containers[i];
                    break;
                }
            }
            this.container = container;
            this._buildNotification(options);
        },
        open: function (options) {
            var opts = $.extend({}, this.options, options);
            this._createNotice(opts);
        },
        _buildNotification: function (options) {
            var $container,
                $notification,
                notification,
                notificationId,
            ...
            $notification.attr('id', notificationId);
            if (this._hasContent(options)) {
                $notification.addClass('notice-notification');
                if (options.type === "light") {
                    $notification.addClass('b');
                }
            } else {
                $notification.addClass('notice-notificationNoContent');
            }
            if (options.verticalEdge === 'bottom') {
                $notification.addClass('up');
            } else {
                $notification.addClass('down');
            }
            $notification.addClass('bg-' + options.type);
            $notification.append(this._buildIcon(options));
            $notification.append(this._buildContent(options));
            $container.append($notification);
            this._playAudio({url: options.audio, volume: options.volume});
            if ($.isFunction(options.onCreate) === true) {
                options.onCreate.call();
            }
            self = this;
            target = $('#' + options.horizontalEdge + '' + options.verticalEdge + '>' + '#' + notificationId);
            if (options.autoClose) {
                setTimeout(function (dom, opts) {
                    if (target) {
                        self._closeNotification(dom, opts);
                    }
                }, options.life, $notification, options);
            }
            ...
            target.find('.rightArea .content').on('click', {
                inst: this,
                opts: options
            }, function (event) {
                var data = event.data;
                if ($.isFunction(data.opts.clickEvent) === true) {
                    data.opts.clickEvent.call(data.inst.element);
                }
            });
        },
        _hasIcon: function (options) {
            return (typeof(options.icon) === "string" && options.icon !== "");
        },
        _buildIcon: function (options) {
            if (this._hasIcon(options)) {
                var iconContainer = $('<div class="pull-left text-center"></div>');
                var icon = $("<i class=" + options.icon + "/>");
                icon.addClass('text-2x');
                if (!this._hasContent(options)) {
                    icon.addClass('notice-pos-center');
                    iconContainer.addClass('nonotice-iconNoContent');
                } else {
                    iconContainer.addClass('notice-top-icon');
                }
                iconContainer.append(icon);
                return iconContainer[0].outerHTML;
            } else {
                return "";
            }
        },
        _buildContent: function (options) {
            var rightArea = $("<div></div>"),
                content = $("<div class='pull-left'></div>"),
                titleDiv = $("<div></div>"),
                detailDiv;
            if (this._hasContent(options)) {
                titleDiv.addClass('m-t-xs');
                rightArea.addClass('rightArea');
                if (options.type === "light") {
                    rightArea.addClass('b-l');
                }
            } else {
                titleDiv.addClass('notice-pos-center');
                rightArea.addClass('rightAreaNoContent');
            }
            titleDiv.append('<span class="text-base font-medium" title="' + (options.title || "Tips") + '">' + (options.title || "Tips") + '</span>');
            content.append(titleDiv);
            if (this._hasContent(options)) {
                detailDiv = $("<div class='m-t-xs m-b-xs notice-subText'><span class='text-base'></span></div>");
                detailDiv.find("span").text(options.content);
                content.append(detailDiv);
            }
            rightArea.addClass(options.type + " " + "lt pull-right");
            content.addClass('content');
            rightArea.append(content);
            rightArea.append(this._buildClose(options));
            return rightArea[0].outerHTML;
        },
        _buildClose: function (options) {
            var container = $('<div class="pull-right m-r-sm"></div>');
            var close = $('<a><i class="md-close"></i></a>');
            if (this._hasContent(options)) {
                container.addClass('notice-top-close');
            } else {
                close.addClass('notice-pos-center');
            }
        ...
        _playAudio: function (options) {
            ...
            if (!$this._audio[options.url]) {
                var audio = jQuery('<audio/>');
                jQuery('<source/>', {src: options.url + '.mp3'}).appendTo(audio);
                jQuery('<source/>', {src: options.url + '.ogg'}).appendTo(audio);
                $this._audio[options.url] = audio[0];
            }
            // Set volume and play audio
            $this._audio[options.url].volume = Math.min((options.volume !== undefined ? options.volume : ($this.options.volume !== undefined ? $this.options.volume : 100) / 100), 1);
            $this._audio[options.url].pause();
            try {
                $this._audio[options.url].currentTime = 0;
            } catch (e) {
            }
            $this._audio[options.url].play();
        },
        _hasContent: function (options) {
            if ((typeof options.content === "string") && (options.content !== '')) {
                return true;
            } else {
                return false;
            }
        },
        _closeNotification: function (n, opts) {
            var num = this.container.data("notice");
            n.fadeOut('slow', function () {
                    n.remove();
                }
            );
            num--;
            if ($.isFunction(opts.onClose) === true) {
                opts.onClose.call(this.element);
            }
            this.container.data("notice", num < 0 ? 0 : num);
        }
    });
    $(document.body).aeNotice();
    $.Notice = function (options) {
        $(document.body).aeNotice('open', options);
    };
    var obj = {
        success: function (title, content, cbClick, cbClose, cbCreate) {
            if (!!content && !!title) {
                title += (": " + content);
            } else if (!!content) {
                title = content;
            }
            var i18n = $.ae.lang.aeNotice;
            $.Notice({
                title: title || i18n.success,
                type: "success",
                verticalEdge: "top",
        ...
        message: function (title, content, cbClick, cbClose, cbCreate) {
            var i18n = $.ae.lang.aeNotice;
            $.Notice({
                title: title || i18n.message,
                type: "light",
                content: content,
                icon: "message_text_outline2",
                clickEvent: $.isFunction(cbClick) ? cbClick : null,
                onClose: $.isFunction(cbClose) ? cbClose : null,
                onCreate: $.isFunction(cbCreate) ? cbCreate : null,
    ...
    $.Notice.success = obj.success;
    $.Notice.info = obj.info;
    $.Notice.warning = obj.warning;
    $.Notice.error = obj.error;
    $.Notice.message = obj.message;
    $.Notice.messageDark = obj.messageDark;
    $.Notice.notify = obj.notify;
    $.Notice.annunciate = obj.annunciate;
    $.Notice.rss = obj.rss;
});
 ...
 */
 define('ui-htree', function(require, exports, moudles) {
    var $=jQuery,
        CSS_OUTLINE="check_box_outline_blank",
        CSS_CHECKED="check_box",
        CSS_INDETERMINATE="indeterminate_check_box",
        CSS_ACTIVE="active",
     ...
     * @property {string} aeHTree.select 选择
     * @property {string} aeHTree.cancel 取消
     */
    $.ae.lang.aeHTree={
        selectedCount:"You have selected ? items",
        select:"Select",
        cancel:"Cancel"
    };
    ...
    function hdSelect(inst,nodeId) {
        var $this=$(this),
            selector="."+CSS_ACTIVE,
            opts=inst.options,
            lazyLoadMode=opts.lazyLoadMode,
            changed,
            hasChildren,
            $el,
            $item,
        ...
        if($el.length>0){
            ...
            if(inst._nodes[orgNodeId].getChildCount()>0){
                $el=$el.find('[data-role="htree-icon"]');
                if(isImgIcon(inst._nodes[orgNodeId].foldedIcon)){
                    $el.attr("src",inst._nodes[orgNodeId].foldedIcon);
                }else{
                    $el.removeClass().addClass(inst._nodes[orgNodeId].foldedIcon);
                }
            }
        }
        ...
        if(nd.pid===null){
            if(changed){
                inst._$sub.empty();
                if(hasChildren){
                    inst._$sub.append(generateList("",inst._getChilrenById(nodeId),opts));
                }
            }else{
                $el=inst._$sub.find('[data-role="htree-list"]:eq(0)');
                ...
                if($item.length>0){
                    ...
                    if(ndCurrent.getChildCount()>0){
                        if(isImgIcon(nd.foldedIcon)){
                            $item.attr("src",nd.foldedIcon);
                        }else{
                            $item.find('[data-role="htree-icon"]').removeClass().addClass(nd.foldedIcon);
                        }
                    }
                }
                $el.nextAll().remove();
            ...
            $el=$this.parents('[data-role="htree-list"]:eq(0)');
            if(changed){
                $el.nextAll().remove();
                if(hasChildren){
                    inst._$sub.append(generateList("",inst._getChilrenById(nodeId),opts));
        ...
        if(opts.mutiSelect===false){
            if(changed){
                if(orgNodeId){
                    affectedNodes=inst._nodes[orgNodeId].setSelected(inst._nodes,inst._levels,false);
                    inst._updateSelectedNodes(affectedNodes);
                }
            }else{
                inst._clearSelectedList();
            }
            nd.setSelected(inst._nodes,inst._levels,true,false,true);
            inst._updateSelectedNodes(inst._getAncestorsById(nodeId,true));
        }
    }
     ...
     * @property {string} nodeState.level 节点名称
     */
    function HTreeNode(nodeState,options) {
        var opts=(options || {});
        this.id=nodeState.id;
        this.pid=nodeState.pid || null;
        ...
        this.isIndeterminate=!!nodeState.isIndeterminate;
        //是否是懒加载节点，如果为true则表示此节点可能存在子节点但是还没有加载
        this.isLazy=((nodeState.isLazy===undefined)?true:!!nodeState.isLazy);
        this.loading=false;
    }
    HTreeNode.prototype={
        /**
        ...
        /**
         * 是否是根节点
         * @return {boolean} 如果是根节点返回true，否则返回false
         */
        isRoot:function(){
        ...
        getState:function() {
            ...
            children=this.childInfo.children,
            i,
            count;
            if(children){
                for(i=0,count=children.length;i<count;i++){
                    state.children.push(children[i].id);
                }
            }
            return state;
        },
        ...
        each:function(nodes,callback,noRelativeAncestors,noRelativeDescendants){
            ...
            if(!noRelativeAncestors){
                nd=this;
                nd=nodes[nd.pid];
                while(nd){
                    nd.each(nodes,callback,noRelativeAncestors,noRelativeDescendants);
                    nd=nodes[nd.pid];
                }
            }
            if(!noRelativeDescendants){
                if(children!==null){
                    for(i=children.length-1;i>=0;i--){
                        children[i].each(nodes,callback,noRelativeAncestors,noRelativeDescendants);
                    }
                }
            }
        },
        ...
        setSelected:function(nodes,levels,selected,noRelativeAncestors,noRelativeDescendants){
            ...
            if(this.selected!==boolSelected){
                factor=(boolSelected?1:-1);
                this.selected=boolSelected;
                this.isIndeterminate=false;
                affectedNodes.push(this);

                levels[this.level].selectedCount+=factor;

                if(this.isRoot()===false){
                    nd=nodes[this.pid];
                    nd.childInfo.selectedCount+=factor;
                    if(!noRelativeAncestors){
                        while(nd){
                            changed=false;
                            state=getCheckCSS(nd.childInfo);
                            ancestorSelected=(state!==CSS_OUTLINE);
                            ancestorIndeterminate=(state===CSS_INDETERMINATE);
                            factor=(ancestorSelected?1:-1);
                            if(nd.selected!==ancestorSelected){
                                changed=true;
                                nd.selected=ancestorSelected;
                                levels[nd.level].selectedCount+=factor;
                                if(nodes[nd.pid]){
                                    nodes[nd.pid].childInfo.selectedCount+=factor;
                                }
                            }
                            if(nd.isIndeterminate!==ancestorIndeterminate){
                                changed=true;
                                nd.isIndeterminate=ancestorIndeterminate;
                            }
                            if(changed){
                                affectedNodes.push(nd);
                            }
                            nd=nodes[nd.pid];
                        }
                    }
                ...
                children=this.childInfo.children;
                if(!noRelativeDescendants && children){
                    for(i=0,count=children.length;i<count;i++){
                        affectedChildNodes=children[i].setSelected(nodes,levels,boolSelected,true);
                        affectedNodes=affectedNodes.concat(affectedChildNodes);
                    }
                }
            }
            return affectedNodes;
    ...
    function getCheckCSS(childInfo) {
        ...
        if(children!==null){
            ...
            for(i=count-1;i>=0;i--){
                if(children[i].isIndeterminate){
                    return CSS_INDETERMINATE;
                }
            }
        }

        if(count===childInfo.selectedCount){
            return CSS_CHECKED;
        }else if(childInfo.selectedCount===0){
            return CSS_OUTLINE;
        }else{
            return CSS_INDETERMINATE;
        }
    ...
    function generateSkeleton() {
        var s='<div class="node-selection-search" data-role="htree-search-bar"><i class="close_circle pull-right none" data-role="htree-search-clear"></i><i class="search"></i><input type="text" class="node-search-input" data-role="htree-search"/></div><ul class="node-selection-result none" data-role="htree-result"></ul><div class="node-selection-tree" data-role="htree-tree"><div class="main-column" data-role="htree-main-list"></div><div class="branch-columns" data-role="htree-sub-list"></div></div>';
        s+=getFooterString();
        return $(s);
    }
    //获得页脚部分的DOM结构字符��?
    function getFooterString() {
        var i18n=$.ae.lang.aeHTree,
            ...
            sCount='<a tabindex="0" role="button" data-toggle="popover" data-trigger="click" class="text-primary font-medium" date-role="htree-count">0</a>';

        s+=i18n.selectedCount.replace("?",sCount);
    ...
    //生成每一层节点的列表容器DOM对象
    function generateList(title,nodes,options){
        var nodeId,
            nd,
            s='<div class="node-selection-tree-column thin-scroll" data-role="htree-list"><h5 class="column-name '+(title || 'none')+'">'+(title || "")+'</h5><ul>';

        for(nodeId in nodes){
            nd=nodes[nodeId];
            s+=getNodeString(nd,options);
        }
    ...
    function getNodeString(node,options){
        var icon,
            ...
            render=options.nodeRender,
            s;
        if(node.isIndeterminate){
            cls=CSS_INDETERMINATE;
        }else{
            cls=(node.selected?CSS_CHECKED:CSS_OUTLINE);
        }
        if(options.lazyLoadMode && node.isLazy===true){
            icon=node.blurryIcon;
        }else if(node.getChildCount()===0){
            icon=node.leafIcon;
        }else{
            icon=node.foldedIcon;
        }
        ...
        if($.isFunction(render)===false){
            s+='<label data-role="htree-label" title="'+node.name+'">'+node.name+'</label></li>';
        }else{
            s+=render(node.getState())+'</li>';
        }
        return s;
    }
    //生成返回结果项的DOM对象
    function generateResultItem(inst,node){
        ...
        if(node.selected){
            checkIcon=((node.isIndeterminate)?CSS_INDETERMINATE:CSS_CHECKED);
            itemIcon=(node.expandedIcon || options.expandedIcon);
        }else{
            checkIcon=CSS_OUTLINE;
            itemIcon=(node.foldedIcon || options.foldedIcon);
        }
        s='<li><div class="result-name" data-node-id="'+node.id+'"><i class="'+checkIcon+' '+(options.mutiSelect?"":"none")+'" data-role="htree-result-checkbox"></i>';
        if($.isFunction(render)){
            s+=render(node.getState())+"</div>";
        }else{
            s+='<i class="'+itemIcon+'"></i><label for="" class="text-c-c">'+node.name+'</label></div>';
        }
        ancestors=inst._getAncestorsById(node.id);
        s+='<div class="result-route">';
        for(i=ancestors.length-1;i>=0;i--){
            nd=ancestors[i];
            s+='<a href="javascript:void(0)" data-role="htree-link" data-node-id="'+nd.id+'">'+nd.name+'</a>';
            if(i>0){
                s+='<i class="chevron_right"></i>';
            }
        }
        s+='</div></li>';
        return $(s);
    ...
    $.aeWidget("ae.aeHTree", {
        ...
        options: {
            ...
            beforeCheck:null,
            beforeUnCheck:null,
            beforeDeleteNode:null,
            afterDeleteNode:null,
            check:null,
            unCheck:null
        },
        _create: function() {
            ...
            $el.attr("aeId",$el.attr("id")).addClass(CSS_CNTR).addClass("pos-rlt");
            this._hdTimer=null;
            this._roots={};
            this._nodes={};
            this._levels=[];
            ...
            $el.append(generateSkeleton());
            this._$input=$el.find('[data-role="htree-search"]');
            this._$tree=$el.find('[data-role="htree-tree"]');
            this._$result=$el.find('[data-role="htree-result"]');
            ...
            if(opts.searchable===false){
                opts.searchable=!opts.searchable;
                this._setOption("searchable",!opts.searchable);
            }else{
                this._addSearchbarEvents();
            }
            if(opts.footerEnabled===false){
        ...
        _refreshResults:function(){
            ...
            value=this._$input.val();
            if(value===""){
                this.element.find('[data-role="htree-search-clear"]').hide();
                this._$tree.show();
                this._$result.hide();
            }else{
                if(value.length>=this.options.minLength){
                    nodes=this._search(value);
                    if(nodes.length>0){
                        this._reloadResult(nodes);
                    }
                }
                this._$tree.hide();
                this._$result.show();
        ...
        _addEvents:function(){
            this.element.find('[data-role="htree-tree"]').on("click.aeHTree",'li',this,function(event){
                var inst=event.data,
                    nodeId=$(this).attr("data-node-id");
                hdSelect.apply(this,[inst,nodeId]);
                inst._trigger("select",null,inst._nodes[nodeId].getState());
            }).on("click.aeHTree",'[data-role="htree-checkbox"]',this,function(event){
                var inst=event.data,
                    $this=$(this),
                    nodeId=$this.parent().attr("data-node-id"),
                    i,
                    count,
                    selected=$this.hasClass(CSS_OUTLINE),
                    affectedNodes,
                ...
                if(inst._trigger((selected?"beforeCheck":"beforeUnCheck"),null,inst._nodes[nodeId].getState())===false){
                    event.stopPropagation();
                    return false;
                }

                affectedNodes=hdCheck.apply(this,[inst,nodeId]);
        ...
        _delEvents:function(){
            this.element.find('[data-role="htree-tree"]').off(".aeHTree");
        },
        _addFooterEvents:function(){
            this._$footer.on("click.aeHTree","button",this,function(event){
                var $this=$(this),
                    inst=event.data;
                if($this.attr("data-role")==="htree-ok"){
                    inst._trigger("ok",null,inst.getSelectedNodes());
                }else{
                    inst._trigger("cancel");
                }
                return false;
                ...
                if(mutiSelect && inst._trigger("beforeUnCheck",null,inst._nodes[nodeId].getState())===false){
                    return;
                }
        ...
        _delFooterEvents:function(){
            this._$footer.off(".aeHTree");
        },
        _addSearchbarEvents:function(){
            this.element.find('[data-role="htree-search-clear"]').on("click.aeHTree",this,function(event){
                var inst=event.data;
                if(inst._hdTimer!==null){
                    clearTimeout(inst._hdTimer);
                    inst._hdTimer=null;
                ...
                inst._$result.hide();
                inst._$tree.show();
                inst._$input.val("")[0].focus();
                event.stopPropagation();
            });
            this._$input.on("keyup.aeHTree",this,function(event){
                var inst=event.data;
                ...
                if(event.which===27){
                    $(this).trigger("blur.aeHtree",inst);
                    return;
                }
                inst.element.find('[data-role="htree-search-clear"]').show();
                inst._hdTimer=setTimeout($.proxy(inst._refreshResults,inst),inst.options.delay);
            }).on("focus.aeHTree",this,function(event){
                var inst=event.data;
                if(inst._$result.has("i").length>0 && inst._$result.css("display")==="none"){
                    inst._$tree.hide();
                    inst._$result.slideDown(200);
                }
            }).on("blur.aeHTree",this,function(event){
                // var inst=event.data;
                // if(inst._$result.css("display")!=="none"){
                //     inst._$result.hide();
                //     inst._$tree.show();
                // }
            });
            this._$result.on("click.aeHTree",'[data-role="htree-result-checkbox"]',this,function(event){
                var inst=event.data,
                    $this=$(this),
                    mutiSelect=inst.options.mutiSelect,
                    selected,
                ...
                if(mutiSelect && inst._trigger((selected?"beforeCheck":"beforeUnCheck"),null,nd.getState())===false){
                    event.stopPropagation();
                    return false;
                }
                $el=inst._$tree.find('[data-node-id="'+nodeId+'"]>[data-role="htree-checkbox"]');

                if($el.length>0){
                    affectedNodes=hdCheck.apply($el[0],[inst,nodeId]);
                }else{
                    affectedNodes= nd.setSelected(nodes,levels,selected);
                    inst._updateSelectedNodes(affectedNodes);
                    for(i=affectedNodes.length-1;i>=0;i--){
                        $el=inst._$tree.find('[data-node-id="'+affectedNodes[i].id+'"]');
                        if($el.length>0){
                            if(affectedNodes[i].isIndeterminate){
                                cls=CSS_INDETERMINATE;
                            }else{
                                cls=affectedNodes[i].selected?CSS_CHECKED:CSS_OUTLINE;
                            }
                            $el.find('[data-role="htree-checkbox"]').removeClass().addClass(cls);
                        }
                    }
                }

                $chks=inst._$result.find('[data-role="htree-result-checkbox"]');
                for(i=$chks.length-1;i>=0;i--){
                    $el=$($chks[i]);
                    $el.removeClass();
                    itemId=$el.parent().attr("data-node-id");
                    if(nodes[itemId].isIndeterminate){
                        $el.addClass(CSS_INDETERMINATE);
                    }else{
                        $el.addClass(nodes[itemId].selected?CSS_CHECKED:CSS_OUTLINE);
                    }
                }
                ...
                var $this=$(this),
                    nodeId,
                    inst=event.data;
                if($this.attr("data-role")==="htree-link"){
                    nodeId=$this.attr("data-node-id");
                    inst.goto(nodeId);
                    inst._$result.hide();
                    inst._$tree.show();
        ...
        _delSearchbarEvents:function(){
            this._$input.off(".aeHTree");
            this._$result.off(".aeHTree");
            this.element.find('[data-role="htree-search-clear"]').off(".aeHTree");
        },
        _updateSelectedNodes:function(nodes,isClear){
            var nd,
                nodeId,
                i,
                count,
            ...
            if(isClear===true){
                for(i=0,count=nodes.length;i<count;i++){
                    nd=nodes[i];
                    if(nd.id in selectedNodes){
                        delete selectedNodes[nd.id];
                        this._selectedCount--;
                    }
                }
            }else{
                $el=this._$footer.find('[data-role="htree-popover"]');
                for(i=0,count=nodes.length;i<count;i++){
                    nd=nodes[i];
                    nodeId=nd.id;
        ...
        _clearSelectedList:function(){
            ...
            this._selectedCount=0;
            this._$footer.find('[data-role="htree-popover"]').empty();
        },
        _search:function(condition){
            ...
            for(nodeId in nodes){
                nd=nodes[nodeId];
                if(nd.name.indexOf(condition)>=0){
                    arr.push(nd);
                }
            }
            return arr;
        },
        _reloadResult:function(nodes){
            var $result=this._$result,
                nd,
                i,
                count=nodes.length;
            $result.empty();
            for(i=0;i<count;i++){
                nd=nodes[i];
                $result.append(generateResultItem(this,nd));
            }
        },
        /**
         * 跳转到指定节��?
         ...
         * @return {boolean} 如果跳转成功返回true，否则返回false
         */
        goto:function(nodeId){
            ...
            if(nodeId in this._nodes){
                ...
                this._$sub.scrollLeft($el.position().left);
                return true;
            }
            return false;
        },
        /**
         * 获得选中节点总数
         * @name ae.aeHTree#getSelectedCount
         * @function
        ...
        _setSelected:function(nodeId,selected,noRelativeAncestors,noRelativeDescendants){
            ...
            if(nodeId in this._nodes){
                ...
                isRoot=nd.isRoot();
                $el=this._$tree.find('[data-node-id="'+nodeId+'"]');
                $chk=$el.find('[data-role="htree-checkbox"]');
                if(selected){
                    $chk.removeClass(CSS_OUTLINE).addClass(CSS_CHECKED);
                }else{
        ...
        _initData:function(data) {
            var i,
                ...
                foldedIcon=this.options.foldedIcon,
            ...
            for(i=0;i<count;i++){
                nd=new HTreeNode({
                    ...
                    pid:data[i].pid,
                    name:data[i].name,
                    expandedIcon:(data[i].expandedIcon || expandedIcon),
                    foldedIcon:(data[i].foldedIcon || foldedIcon),
                    leafIcon:(data[i].leafIcon || leafIcon),
                    blurryIcon:(data[i].blurryIcon || blurryIcon),
            ...
            for(nodeId in nodes){
                nd=nodes[nodeId];
                if(nd.pid!==null){
                    ndParent=nodes[nd.pid];
                    if(ndParent.childInfo.children===null){
                        ndParent.childInfo.children=[];
                    }
            ...
            cb=function(nds){
                var levelInfo;
                if(this.isRoot()===false){
                    this.level=nds[this.pid].level+1;
                    this.rootId=nds[this.pid].rootId;
                }
                ...
                if(!levelInfo){
                    levelInfo=levels[this.level]={
                        selectedCount:0,
                        count:0,
                        parents:{} //放置本层节点的所有父节点实例
                    };
                }
                if(this.selected){
                    levelInfo.selectedCount++;
                }
                levelInfo.count++;
                if(this.pid!==null){
                    levelInfo.parents[this.pid]=nodes[this.pid];
                }
            };
            for(nodeId in roots){
        ...
        _filterSubTree:function(){
            var cache,
                ...
                cb,
                levelInfo,
                subTreeId=this.options.subTreeId,
                nodeId,
                self=this;
            if(subTreeId!==null && (subTreeId in this._nodes)){
                cache={};
                for(nodeId in this._nodes){
                    cache[nodeId]=this._nodes[nodeId];
                }
                nodes=this._nodes={};
                roots=this._roots={};
                levels=this._levels=[];
                ...
                newRootId=cache[subTreeId].id;
                cb=function(nodeSet){
                    if(this.rootId==oldRootId){
                        nodes[this.id]=this;
                        if(this.isRoot()===false){
                            this.level=nodeSet[this.pid].level+1;
                        ...
                        if(this.pid!==null){
                            levelInfo.parents[this.pid]=nodeSet[this.pid];
                        }
                    }
                };
                cache[subTreeId].pid=null;
                cache[subTreeId].level=0;
                cache[subTreeId].each(cache,cb,true);
                cache[subTreeId].rootId=cache[subTreeId].id;
                roots[subTreeId]=cache[subTreeId];
            }
        },
        _setOptions:function(options) {
            ...
            this._super(options);
            opts=this.options;
            if(prevDisabled!==opts.disabled){
                if(opts.disabled){
                    this.element.find('[data-role="htree-search-clear"]').trigger("click.aeHTree",this);
                    this._delEvents();
                    this._delSearchbarEvents();
                    this._delFooterEvents();
                    this._$input.prop("disabled",true).addClass("bg-gray");
                    this._$footer.find('[data-role="htree-cancel"],[data-role="htree-ok"]').prop("disabled",true);
                    this._$footer.find('[data-role="htree-tips"]').addClass("text-muted").find("a").removeClass("text-primary").addClass("text-muted");
                }else{
                    this._$input.prop("disabled",false).removeClass("bg-gray");
                    this._$footer.find('[data-role="htree-cancel"],[data-role="htree-ok"]').prop("disabled",false);
                    this._$footer.find('[data-role="htree-tips"]').removeClass("text-muted").find("a").removeClass("text-muted").addClass("text-primary");
                    this._addEvents();
                    if(opts.searchable){
                        this._addSearchbarEvents();
                    }
        ...
        _setOption: function( key, value ) {
            ...
            switch (key) {
                case "searchable":
                    $el=this.element.find('[data-role="htree-search-bar"]');
                    if(value){ //启用
                        this._addSearchbarEvents();
                        $el.show();
                        this._$tree.css("padding-top","auto");
                    }else{ //禁用
                        this._$result.hide();
                        this._$tree.show();
                        $el.hide();
                        this._delSearchbarEvents();
                        this._$tree.css("padding-top",0);
                    }
                    break;
                case "mutiSelect":
                ...
                case "source":
                    if($.isArray(value)){
                        ...
                        this.reload(value.call(this.element,null));
                    }
                    break;
                case "footerEnabled":
                    if(value){
                        ...
                        this._delFooterEvents();
                        this._$footer.hide();
                        this._$tree.css("padding-bottom",0);
                    }
                    break;
                case "minLength":
                    if($.isNumeric(value)){
                        this.options.minLength=((value<1)?1:parseInt(vlaue,10));
                    }else{
        ...
        addLazyChilds:function(parentId,serviceName,params,convertor){
            ...
            $.ajaxPost(serviceName,params,$.proxy(function(data){
                var item,
                    i,
                    count,
                    $el,
                ...
                if($.isFunction(convertor) && finalData!==null){
                    count=finalData.length;
                    for(i=0;i<count;i++){
                        item=convertor(finalData[i]);
                        this.addNode({
                            id:item.id,
                            pid:parentId,
                            name:item.name,
        ...
        _checkState:function(nodeState){
            var b=(!(nodeState.id in this._nodes) && nodeState.id && nodeState.name);
            if(nodeState.pid){
                b=(b && (nodeState.pid in this._nodes));
            }
            return !!b;
        },
        /**
        ...
        addNode:function(nodeState){
            var nodes=this._nodes,
                ...
                ancestors,
                isRoot;
            if(this._checkState(nodeState)===true){
                nd=new HTreeNode(nodeState,this.options);
                isRoot=(nd.pid===null);
                nd.level=(isRoot?0:(nodes[nd.pid].level+1));
                levelInfo=levels[nd.level];
                if(!levelInfo){
                    levelInfo=levels[nd.level]={
                        selectedCount:0,
                ...
                if(isRoot===false){
                    levelInfo.parents[nd.pid]=nodes[nd.pid];
                    childInfo=nodes[nd.pid].childInfo;
                    if(nodes[nd.pid].getChildCount()===0){
                        childInfo.children=[];
                    }
                    childInfo.children.push(nd);
                    if(nd.selected===true){
                        levelInfo.selectedCount++;
                        childInfo.selectedCount++;
                    }
                    this._updateAncestorNodesState(nodes[nd.pid]);
                }
                nodes[nd.id]=nd;
                if(nd.pid===null){
                    $list=this._$main.find('[data-role="htree-list"]:eq(0)');
                    $list.find('ul:eq(0)').append($(getNodeString(nd,this.options)));
                }else{
                    this._refreshAssociationNodesView(nd);
                }
                if(nd.selected){
                    this._updateSelectedNodes([nd]);
                }
                return true;
            }
            return false;
        },
        _refreshAssociationNodesView:function(nd) {
            var opts=this.options,
                lazyLoadMode=opts.lazyLoadMode,
                changed,
                selector="."+CSS_ACTIVE,
            ...
            if($el.length>0){
                if($el.hasClass(CSS_ACTIVE)===true){
                    ...
                    if($list.length>0){
                        $list.find('ul:eq(0)').append($(getNodeString(nd,opts)));
                    }else{
                        this._$sub.append(generateList("",this._getChilrenById(nd.pid),this.options));
                    }
                }else{
                    $el.addClass(CSS_ACTIVE).siblings(selector).removeClass(CSS_ACTIVE);
                    $el.parents('[data-role="htree-list"]:eq(0)').next().remove();
                    this._$sub.append(generateList("",this._getChilrenById(nd.pid),this.options));
                }
            }else{
                ancestors=this._getAncestorsById(nd.pid);
                count=ancestors.length;
                for(i=0;i<count;i++){
                    this._refreshNodeView(ancestors[i]);
                }
            }
        },
        _updateAncestorNodesState:function(node){
            var cls,
                nd=node,
                nodes=this._nodes;
            ...
            while(nd){
                css=getCheckCSS(nd.childInfo);
                nd.isIndeterminate=(css===CSS_INDETERMINATE);
                nd.selected=(css!==CSS_OUTLINE);
                this._refreshNodeView(nd);
                nd=nodes[nd.pid];
            }
        },
        _refreshNodeView:function(node){
            var $el,
                $label,
                icon,
                cls,
            ...
            $el=this._$tree.find('[data-node-id="'+nodeId+'"]');
            if($el.length>0){
                if(this.options.lazyLoadMode && node.isLazy===true){
                    icon=node.blurryIcon;
                }else if(node.getChildCount()===0){
                    icon=node.leafIcon;
                }else{
                ...
                if(isImgIcon(icon)){
                    $el.find('[data-role="htree-icon"]').attr("src",icon);
                }else{
                    $el.find('[data-role="htree-icon"]').removeClass().addClass(icon);
                }
                if(this.options.mutiSelect===true){
                    if(node.isIndeterminate){
                        $el.find('[data-role="htree-checkbox"]').removeClass().addClass(CSS_INDETERMINATE);
                    }else{
                        $el.find('[data-role="htree-checkbox"]').removeClass().addClass(node.selected?CSS_CHECKED:CSS_OUTLINE);
                    }
                }
                $label=$el.find('[data-role="htree-label"]');
                if($label.text()!==node.name){
        ...
        deleteNode:function(nodeId){
            ...
            if(nodeId in nodes){
                if(this._trigger("beforeDeleteNode",null,nodes[nodeId].getState())===false){
                    return;
                }
                nd=nodes[nodeId];
                this._deleteNodeView(nd);
                if(nd.pid!==null){
                    nd=nodes[nd.pid];
                    nd.childInfo.children=null;
                    nd.childInfo.selectedCount=0;
                    nd.isLazy=false;
                    nd.loading=false;
                }
                deleteNodes=[];
                ...
                cb=function(nodes){
                    ...
                    if(nodeId in levelInfo.parents){
                        delete levelInfo.parents[nodeId];
                    }
                    deleteNodes.push(this);
                };
                nd.each(nodes,cb,true);
                //更新选中总数
                this._updateSelectedNodes(deleteNodes,true);
                //然后释放节点实例
                for(i=deleteNodes.length-1;i>=0;i--){
                    deleteNodes[i].gc();
        ...
        _deleteNodeView:function(node){
            var $el;
            if(node.isRoot()){
                this._$main.find("[data-node-id='"+node.id+"']").remove();
                if(node.selected===true){
                    this._$sub.empty();
                    this.reset();
                }
            }else{
                $el=this._$sub.find("[data-node-id='"+node.id+"']");
                $el.parents('[data-role="htree-list"]:eq(0)').nextAll().remove();
                $el.remove();
            }
        },
        setNodeText:function(nodeId,text){
            if($.isFunction(this.options.nodeRender)){

            }else{
                this._$tree.find('[data-node-id="'+nodeId+'"]').find("label").text(text);
            }
        },
        /**
         * 清除节点
         * @name ae.aeHTree#clear
         * @function
         */
        clear:function(){
            var nodeId,
            ...
            this.element.find('[data-role="htree-search-clear"]').trigger("click.aeHTree",this);
            for(i=this._levels.length-1;i>=0;i--){
                parents=this._levels.parents;
        ...
        getNodeById:function(nodeId){
            if(nodeId in this._nodes){
                return this._nodes[nodeId].getState();
            }
            return null;
        },
        /**
         ...
         */
        reset:function(){
            var nodeId=this._$main.find('[data-node-id]:eq(0)').attr("data-node-id");
            this.goto(nodeId);
        },
        /**
         * 获得选中节点状�??
        ...
        _getAncestorsById:function(nodeId,isSelected,isObverse){
            var nodes=this._nodes,
                nd=nodes[nodeId],
                arr=[];
            if(isSelected){
        ...
        getAncestorsById:function(nodeId,isSelected,isObverse){
            var nodes=this._getAncestorsById(nodeId,isSelected,isObverse),
                ...
                count=nodes.length;
            for(i=0;i<count;i++){
                nodes[i]=nodes[i].getState();
            }
            return nodes;
        },
        _getChilrenById:function(nodeId,isSelected){
            var nd=this._nodes[nodeId],
                children,
                obj={},
                i,
                count;
            if(nd.getChildCount()>0){
                children=nd.childInfo.children;
                if(isSelected){
                    for(i=0,count=children.length;i<count;i++){
        ...
        _destroy:function(){
            var nodeId,
                $el=this.element,
                aeInit=$el.data("org_aeInit");

            this.clear();
            if(aeInit){
                $el.attr("aeInit",aeInit);
            }
            $el.removeAttr("aeId").removeClass(CSS_CNTR);
